...
<?php
	...
	if($paymethod == 'paypal'){
		...
		$link=$CFG->wwwroot. '/portal/subscribe/telegraphic_pay.php?paymethod=paypal&&traineeID='.$traineeID.'&&trainee_name='.$firstname.'&&lastname='.$lastname.'&&address='.$address.'&&address2='.$address2.'&&email='.$email.'&&phone='.$phone.'&&city='.$city.'&&province='.$province.'&&postal='.$postal.'&&country='.$country.'&&cost='.$cost.'&&currency='.$currency.'&&PaypalBusinessEmail='.$PaypalBusinessEmail.'&&coursename='.$coursename.'&&shortname='.$shortname.'&&courseid='.$courseid; 
		...
		$link2=$CFG->wwwroot. '/portal/subscribe/telegraphic_pay.php?paymethod=creditcard&&traineeID='.$traineeID.'&&trainee_name='.$firstname.'&&lastname='.$lastname.'&&address='.$address.'&&address2='.$address2.'&&email='.$email.'&&phone='.$phone.'&&city='.$city.'&&province='.$province.'&&postal='.$postal.'&&country='.$country.'&&cost='.$cost.'&&currency='.$currency.'&&PaypalBusinessEmail='.$PaypalBusinessEmail.'&&coursename='.$coursename.'&&shortname='.$shortname.'&&courseid='.$courseid; 
		}else{
...
<ol style="list-style-type:upper-alpha; text-align: justify;">
	...
	<li><strong>E-Learning Users are NOT  Allowed to teach using ANY of <?=$policyname;?>Online Training Program:</strong></li><br/>
		<p>You are not allowed to teach or  instruct using <?=$policyname;?> e-Learning as instructional tools for ANY  REASON without the written permission from SHAPETM. Doing this is  equivalent to sharing account access, and as stated above this is strictly  prohibited.<strong></strong></p>  
