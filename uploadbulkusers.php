...
if ($studentnow <= '0') {
    ...
    while ($chatting = mysql_fetch_array($sqlchat)) {
        ...
        $today = strtotime('now');
        $expiring_date = strtotime('now + 150 days'); //expiring date
        ...
        if ($cusertoken == '0') {
            ...
            $tokenexpiry = strtotime(date('d-m-Y H:i:s', $tokencreated) . " + 60 days"); //token expiry
    ...
    $preference->userid = $usercandidateid;
    $preference->name   = 'create_password';
    $preference->value  = '1';
