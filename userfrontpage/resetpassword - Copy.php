...
<?php
	...
	if($USER->id !='2'){
		...
		$statement="mdl_cifauser WHERE 	deleted='0' AND (usertype='Active candidate' OR usertype='inactive candidate')";
	}
	...
	$sqlcourse="SELECT * FROM {$statement} ORDER BY a.id DESC";	
...
?>
  <tr>
	...
	<?php 		
		...
		if($USER->id !='2'){
			$sqluser=mysql_query("SELECT * FROM mdl_cifarole_assignments a, mdl_cifarole b WHERE a.roleid=b.id AND a.userid='".$sqlrow['userid']."'");
		}else{
	...
	//start date
		...
		if($USER->id != '2'){ $getuserid=$sqlrow['id']; }else{ $getuserid=$sqlrow['userid']; }
	?>
	</td>
    <td class="adjacent" style="text-align:center;">
