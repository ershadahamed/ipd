<?php
	...
	$PAGE->navbar->add(get_string('mytraining'), $mytraininglink)->add('Test Result', $urlexamresult);
	...
	?>
		...
		<table id="availablecourse">
			...
			$qryge = "SELECT a.orgtype FROM mdl_cifauser a, mdl_cifaorganization_type b
				...
				$qryed = "SELECT * FROM mdl_cifaquiz_grades WHERE quiz='26' AND userid='".$q['id']."' AND timemodified>'1455494400'";
			...
			//mmn end
				$sgrade=mysql_query("
					...
					Where
					  a.quiz = b.id And a.grade >= '".$passgrade."' And c.visible!='0' And
					  (c.category = '3' And
					  a.userid = '".$q['id']."')				
				");
				$bil=1;
			...
			<tr>
				...
				<?php
					...
					// if (round($qgrade['usergrade']) < '70'){ echo 'u fail'; }
				?>
				</td>
				<?php 
				...
				<td class="adjacent" align="center">
					...
					?>
						<input type="button" id="id_actionbutton" name="statement" value="<?=get_string('cert.');?>" onclick="window.open('<?=$CFG->wwwroot. "/userfrontpage/certify.php?id=".$USER->id."&testid=".$qgrade['quiz'];?>', 'Window2', 'width=850, height=500,resizable = 1, scrollbars=yes');"/>
