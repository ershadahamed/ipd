...
<!--- step by step----->
<style>
    #shopcart img{max-width:80%;margin-left: 3em;}
</style>
...
<table id="avalaiblecourse" style="width:100%; float:left;" border="0" cellpadding="0" cellspacing="0"><tr valign="top">
	...
	";
            if ($search != '') {
                $sqlCourses.=" And (a.fullname LIKE '%" . $search . "%' Or a.shortname LIKE '%" . $search . "%')";
            }
		...
		");
                    ...
                    if (($_GET['resitexamid'] == '3') OR ( $_GET['resitexamid'] == '4')) {   /////// IF EXTEND OR RESIT
                        $sr = mysql_query("SELECT idnumber FROM {$CFG->prefix}course WHERE idnumber='" . $_GET['resitcode'] . "' AND id='" . $rowCourses['courseid'] . "'");
                        $ssr = mysql_num_rows($sr);
                        if ($ssr != '0') {

                            // echo  $rowCourses['idnumber'].$_GET['resitcode'];
                            // if($ssr!='0'){ // echo  $rowCourses['idnumber'].$_GET['resitcode'];
                            ?>  
                            ...
                            <?php if ($rowC != '0') { // ada logo purchase ?>
                                <table style="margin-top:4em; margin-left:3em;" class="hovertable" cellpadding="0" cellspacing="0" border="0">
                                <?php } else { ?>
                                                                ...
                                                                <div class="date" style="text-align:center;">			
                                                                    ...
                                                                    if ($rowC == '0') {
                                                                        if ($sqlgot['paystatus'] == $statuspending) {
                                                                            ?>
                                                                            <input type="image" src="image/buy_now.png" name="buy_now" onclick='alert( & quot; You already subscribe this module <?= ucwords(strtoupper($rowCourses['shortname'])); ?> and status still <?= ucwords(strtoupper($sqlgot['paystatus'])); ?>.\nPlease proceed with your payment OR Please contact administrator for more information. & quot; )' title="Buy Now" width="80" >
                                                                        <?php } else { ?>
            ...
            <?= $rowCourses['summary']; ?>
                                                                ...
                                                                <div class="date" style="text-align:left;">			
                                                                    ...
                                                                    // Display Course Outline & Buy Now Button
                                                                    $ecourse=getListEnrolCourse($rowCourses['idnumber']);
                                                                    $langno=1;
                                                                    foreach($ecourse as $ec){
                                                                        $lang=$langno++;
                                                                        $paymentstatus=statusCoursePayment($ec->courseid, $USER->traineeid);
                                                                        $course_details = $CFG->wwwroot . "/courses_synopsis_pdf/view_course_details.php?id=" . $ec->courseid;
                                                                        $enrolstatus=totalEnrolCourse($USER->id,$ec->enrolid);
                                                                        $onclick="window.open('".$course_details."', 'Window2', 'width=820,height=600,resizable = 1,scrollbars=1');";
                                                                        
                                                                        if($lang=='1'){ $text='EN'; }
                                                                        if($lang=='2') {$text='AR';}                                                                        
                                                                        
                                                                        // if($paymentstatus->paystatus==$statuspending){
                                                                            // $cart="alert( & quot; You already subscribe this module ".ucwords(strtoupper($ec->shortname))." and status still ". ucwords(strtoupper($paymentstatus->paystatus))."\nPlease proceed with your payment OR Please contact administrator for more information. & quot; )";
                                                                        // }else{
                                                                            $cart="addtocart(".$ec->courseid.")";
                                                                        // }
                                                                        
                                                                        if($lang>1){ echo '<br/>'; }
                                                                        echo '<a class="viewcourse_details" style="color:#FFFFFF;" href="#" onclick="'.$onclick.'"> '.$text.' | &nbsp;View Course Outline </a>';                                                                     
                                                                        if($enrolstatus!=1){ // if not enrol yet, display button BUY
                                                                            echo '<a class="buynowbutton" name="buy_now" style="color:#FFFFFF;" href="#" onclick="'.$cart.'" title="Buy Now"> Buy Now </a>';
                                                                        }  
                                                                        // echo $enrolstatus.$ec->courseid;                                                                     
                                                                    }
                                                                    // END here.
                                                                    ...
                                                                    if ($rowCourses['courseid'] == '59') {
                                                                        ...
                                                                        <input type="image" src="image/view_course_details.png" name="view_course_details" onclick="window.open('<?= $printtxt; ?>', 'Window2', 'status = 1, width=880, height=500, resizable = yes, scrollbars=1');" onMouseOver="style.cursor = 'pointer'" title="View Course Details" width="133px">
                                                                        <?php } /*else { ?>
                                                                            <a class="viewcourse_details" style="color:#FFFFFF;" href="#" onclick="window.open('<?= $course_details; ?>', 'Window2', 'width=820,height=600,resizable = 1,scrollbars=1');"> EN | View Course Outline </a>
                                                                            <?php if ($rowC == '0') { ?><a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick="addtocart(<?= $rowCourses['courseid']; ?>)" title="Buy Now">Buy Now</a><?php } ?><br/>
                                                                            <a class="viewcourse_details" style="color:#FFFFFF;" href="#" onclick="window.open('<?= $course_details; ?>', 'Window2', 'width=820,height=600,resizable = 1,scrollbars=1');"> AR | View Course Outline </a>                                                                     
                                                                            <?php if ($rowC == '0') { ?><a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick="addtocart(<?= $rowCourses['courseid']; ?>)" title="Buy Now">Buy Now</a><?php } ?>
                                                                        <?php } */
                                                                    ...
                                                                    if ($rowC == '0') {
                                                                        if ($sqlgot['paystatus'] == $statuspending) {
                                                                            ?>
                                                                                                <!--input type="image" src="image/buy_now.png" name="buy_now" onclick='alert( & quot; You already subscribe this module <?//= ucwords(strtoupper($rowCourses['shortname'])); ?> and status still <?//= ucwords(strtoupper($sqlgot['paystatus'])); ?>.\nPlease proceed with your payment OR Please contact administrator for more information. & quot; )' title="Buy Now" width="80" -->
                                                                            <a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick='alert( & quot; You already subscribe this module <?= ucwords(strtoupper($rowCourses['shortname'])); ?> and status still <?= ucwords(strtoupper($sqlgot['paystatus'])); ?>.\nPlease proceed with your payment OR Please contact administrator for more information. & quot; )' title="Buy Now">Buy Now</a><br/>
                                                                            <a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick='alert( & quot; You already subscribe this module <?= ucwords(strtoupper($rowCourses['shortname'])); ?> and status still <?= ucwords(strtoupper($sqlgot['paystatus'])); ?>.\nPlease proceed with your payment OR Please contact administrator for more information. & quot; )' title="Buy Now">Buy Now</a>
                                                                        <?php } else { ?>
                                                                            <!--a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick="addtocart(<?= $rowCourses['courseid']; ?>)" title="Buy Now">Buy Now</a>
                                                                            <a class="buynowbutton" style="color:#FFFFFF;" name="buy_now" href="#" onclick="addtocart(<?= $rowCourses['courseid']; ?>)" title="Buy Now">Buy Now</a-->                                                                            
            ...
            ?>
                    ...
                    if ($kirarow == $count) {
                        echo "<h3>All course has been subscribe.</h3>";
                    }
                    ?>
                    </td>
    ...
    <?php // if($kirarow != $count){   ?>
                            <table style="width:100%; background-color:#fff; border:1.8px solid #3D91CB;margin-top:4em;" >
						...
						Where
						  c.id='" . $_GET['resitid'] . "' And a.category = '3' And a.visible = '1' And e.userid='" . $USER->id . "'	
					");
                                                            ...
                                                            <div style="text-align:center;" id="cartsbutton">
                                                                ...
                                                                <?php } else if ($_GET['resitexamid'] == '4') { ?>
                                                                    <input class="proceedbutton" type="button" style="cursor:pointer;" name="clearcart" value="Proceed" title="Proceed to next step" onclick="window.location = '<?= $CFG->wwwroot; ?>/portal/subscribe/paydetails_loggeduser_extend.php?pid=<?= $pid; ?>&resitexamid=4&resitid=<?= $_GET['resitid']; ?>'" >              
                                                                <?php } else { ?>
