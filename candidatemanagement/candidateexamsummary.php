<?php
	...
	$ccmanagement = get_string('cifacandidatemanagement');
    $PAGE->navbar->add(ucwords(strtolower($ccmanagement)));	
...
<?php } ?>	
    ...
    <tr>
      <td align="right">
				<?php		
					$printto=$CFG->wwwroot. '/candidatemanagement/candidateexamsummary_print.php?id='.$_GET['id'].'&examid='.$_GET['examid'];
				?>	  
	  <input type="submit" name="buttonprint" id="buttonprint" value="Print" onClick="this.form.action='<?=$printto;?>', target='_blank'" />
      <input type="submit" name="buttonback" id="buttonback" onClick="this.form.action='<?=$CFG->wwwroot. '/index.php';?>'" value="<?=get_string('back');?>" /></td>
    </tr>    
  </table>
  </fieldset>
</form>
