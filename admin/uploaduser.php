...
require_once('../lib/logactivity_lib.php'); // added by arizan abdullah
...
//$returnurl_2 = new moodle_url('/enrol/users.php', array('id'=>$course->id));
...
$STD_FIELDS = array('id', 'firstname', 'lastname', 'username', 'email', 'mobile', 'gender',
        'city', 'country', 'lang', 'timezone', 'mailformat',
        'maildisplay', 'maildigest', 'htmleditor', 'ajax', 'autosubscribe',
        'institution', 'department', 'idnumber', 'skype',
        'msn', 'aim', 'yahoo', 'icq', 'phone1', 'phone2', 'address', 'address2', 'address3', 'postcode',
        'url', 'description', 'descriptionformat', 'password','traineeid', 'lasttimecreated',
        'dob','usertype','access_token','designation','orgtype','roleid', 'title',
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    $autoenrol      = array(); // cache of used manual enrol plugins in each course
    ...
    while ($line = $cir->next()) {
        ...
        $user->username = generatecadidateid($user->country);
        ...
        if ($existinguser) {
            ...
            // modify by arizan abdullah
            $defaultpassword = get_string('temporarypassword');
            $user->username = generatecadidateid($user->country);
            $user->traineeid = $user->username;
            $user->usertype = 'Active candidate';
            $user->access_token = $user->idnumber;
            $user->dob = strtotime($user->dob);
            $user->designation = ucwords(strtolower($user->designation));
            if($user->gender == 'Male'){ $user->gender = '0'; }else{ $user->gender = '1'; }
            if($user->title){
            if($user->title == 'Mr'){ $user->title = '1'; }elseif($user->title == 'Miss'){ $user->title = '3'; }else{ $user->title = '2'; }
            }else{ if($user->gender == 'Male'){ $user->title = '1'; }else{ $user->title = '3'; } }
            $user->lasttimecreated  = strtotime("+ 120 day", time());   // account active for 4 month
            ...
            if ($isinternalauth) {
                if (empty($user->password)) {
                    if ($createpasswords) {
                        ...
                        $upt->track('password', '', 'normal', false);
                        $upt->track('password', get_string('uupasswordcron', 'admin'), 'warning', false);   // Generated in cron
                    } else {
                        $upt->track('password', '', 'normal', false);
            ...
            // if ($user->password === 'to be generated') { // modified by arizan abdullah
            if ($user->password === hash_internal_user_password($defaultpassword)) {
    ...
    // sent an email after regisration.
    // mtrace('checking for create_password');
    if ($DB->count_records('user_preferences', array('name' => 'create_password', 'value' => '1'))) {
        // mtrace('creating passwords for new users');
        $newusers = $DB->get_recordset_sql("SELECT u.id as id, u.email, u.firstname, u.traineeid,
                                                         u.lastname, u.username, u.orgtype,
                                                         p.id as prefid
                                                    FROM {user} u
                                                    JOIN {user_preferences} p ON u.id=p.userid
                                                   WHERE p.name='create_password' AND p.value='1' 
												   AND u.email !='' AND u.suspended = 0 AND u.auth != 'nologin' AND u.id>1728");

        // note: we can not send emails to suspended accounts
        foreach ($newusers as $newuser) { 

            // Get auto enrol for chat, feedback, mocktest, final test 
            foreach(get_autoenrol() as $cid){ 
                $courseid = $cid->id;
                $today = time();
                if ($instances = enrol_get_instances($courseid, false)) {
                    foreach ($instances as $instance) {
                ...
                // Set Group ID for users // added by arizan abdullah
               /* if(empty($newuser->orgtype)){
                    $groupid = listoutall_groupusers($newuser->id)->id;
                    if($groupid=='1'){ $orgtype='7'; }
                    else if($groupid=='7'){ $orgtype='1'; }
                    $sql = "UPDATE {$CFG->prefix}user SET orgtype='".$orgtype."' WHERE id='".$newuser->id."'";
                    $DB->execute($sql);
                }*/
                ...
                if ($rid) {
                    // find duration
                    $timeend   = 0;
                    $duration = (int)$cduration * 60*60*24; // convert days to seconds
                    if ($duration > 0) { // sanity check
                        $timeend = $today + $duration;
                    }                   
                    //$manual->enrol_user($autoenrol[$courseid], $newuser->id, $rid, $today, $timeend);   // enrol id
					 
                }                                      
            }            
            // end auto enrol for chat, feedback, mocktest, final test
                       // email user
            if (lms_welcome_email($newuser)) {
//mmn to turn on this upon confirmation 
if($newuser->orgtype=='1' || $newuser->orgtype=='3' || $newuser->orgtype=='7' || $newuser->orgtype=='8'){
lms_welcome_email_adib($newuser);
}
		...
		unset_user_preference('create_password', $newuser);
            ...
            // created token for final test
            $finaltestid = '64';
            $access_token = uniqid(rand());
            if($DB->count_records('user_accesstoken',array('userid'=>$newuser->id, 'courseid'=>$finaltestid))=='0'){
                // create token id for final test // by arizan abdullah
                //inser record 
                echo $qryUP = "INSERT INTO {$CFG->prefix}user_accesstoken SET "
                . "user_accesstoken='" . $access_token . "',
                                            timecreated_token='" . time() . "', "
                        . "tokenexpiry='" . strtotime("+ 120 day", time()) . "', "
                        . "userid='" . $newuser->id . "', "
                        . "courseid='" . $finaltestid . "'";
						$sqlUP = mysql_query($qryUP)
                                    or die("Not update" . mysql_error());                
            }            
        }
        $newusers->close();
    }   // end sent mail    
 //mmn add to enrol mock test, final test, feedback, chat 
$today = time();
$cduration2 = $user->enrolperiod1; // 5 month
               
                    // find duration
                    $timeend2   = 0;
                    $duration2 = (int)$cduration2 * 60*60*24; // convert days to seconds
                    if ($duration2 > 0) { // sanity check
                        $timeend2 = $today + $duration2;
                    }                    
                   
	
	echo "==".$sqli1 = "INSERT INTO {$CFG->prefix}user_enrolments (status,enrolid,userid,timestart,timeend) VALUES
	'1','144','".$newuser->id.",'".$today."', '".$timeend2."')";//chat
    $DB->execute($sqli1);
	
	$sqli2 = "INSERT INTO {$CFG->prefix}user_enrolments (status,enrolid,userid,timestart,timeend) VALUES
	'1','186','".$newuser->id.",'".$today."', '".$timeend2."')";//feedback
    $DB->execute($sqli2);
	
	$sqli3 = "INSERT INTO {$CFG->prefix}user_enrolments (status,enrolid,userid,timestart,timeend) VALUES
	'1','207','".$newuser->id.",'".$today."', '".$timeend2."')";//mock test
    $DB->execute($sqli3);
	
	$sqli4 = "INSERT INTO {$CFG->prefix}user_enrolments (status,enrolid,userid,timestart,timeend) VALUES
	'1','312','".$newuser->id.",'".$today."', '".$timeend2."')";//final test
    $DB->execute($sqli4);
/*mmn end*/	
