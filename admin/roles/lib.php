...
class potential_assignees_course_and_above extends role_assign_user_selector_base {
    public function find_users($search) {
		...
		/*$listrole=mysql_query("SELECT id, userid FROM mdl_cifarole_assignments WHERE id!='5'");
		while($rrole=mysql_fetch_array($listrole)){
			$rroleid.=$rrole['userid'];
		}
		$rroleid=$ruserid;*/
