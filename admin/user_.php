<?php
    ...
    if ($extrasql !== '') {
        ...
        echo $OUTPUT->heading("Number of users: "."$usercount ".get_string('users'));
    }
    ...
    if (!$users) {
		...
		//if(($qrole->roleid) == '16'){
       // $table->head = array ($fullnamedisplay, $dob, $traineeid, $usertype/*$lastaccess,*/);
		//}else{
        ...
        foreach ($users as $user) {
			...
			$row=mysql_fetch_array($query);
				...
				$dob=unix_timestamp_to_human($row['dob'] + 3600*24);
			?>
	...
	<?php echo '<div style="padding-bottom:5px;">';
		$title_search = '<b>Name / Trainee ID / City / Country / Usertype :</b>';
		echo $title_search;
		echo '</div>';
	?>
