...
<?php
	...
	$sqlcourse="SELECT * FROM {$statement} WHERE paymethod!=''";
...
?>
  <tr style="<?php if ($pstatus1){ echo $color1; }else if ($pstatus2){ echo $color2; }else{ echo $color3; } ?>">
			...
			if ($sqlrow['paystatus'] == 'new') {
				if($sqlrow['paymethod'] != 'telegraphic'){
					...
					if($today > $expiry){
						...
						?>
						<input onClick="this.form.action='<?=$CFG->wwwroot ."/transaction_status.php?confirmuser=".$sqlrow['customerid']."&orderid=".$sqlrow['orderid']."&datepurchase=".$sqlrow['date'];?>'" onMouseOver="style.cursor='pointer'" type="submit" name="sahbeli" value="<?=get_string('confirm');?>"/>
						<?php
						echo '</div>';
						...
						if($_POST['sahbeli']){						
							...
							while($sgetuser=mysql_fetch_array($selectgetuser)){
								...
								$ucandidates=mysql_query("UPDATE mdl_cifacandidates SET candidateid='".$ucadidateid."' WHERE traineeid='".$yes."'");
								...
								if($q30){
									$ucandidates=mysql_query("UPDATE mdl_cifaorders SET paystatus='paid' WHERE date='".$sgetuser['date']."' AND paymethod='telegraphic' AND customerid='".$q30['serial']."'");
								}
								...
								if($ucount=='0'){
									...
									$sqlInsert=mysql_query("INSERT INTO mdl_cifauser_enrolments 
															SET enrolid='".$getenrolid."', userid='".$ucadidateid."',
	...
	?>
	<input onClick="this.form.action='<?=$CFG->wwwroot ."/transaction_status.php?orderid=".$sqlrow['orderid'];?>'" name="submitimage" type="image" id="submit" value="submit" src="<?=$linkdelete;?>" width="15" title="Delete module subscribed" />
	<?php
	if (isset($_GET['orderid'])) {
		if($_GET['orderid']==$sqlrow['orderid']){
			$sqldelete=mysql_query("UPDATE {$CFG->prefix}orders SET paystatus='cancel' WHERE paystatus='new' AND serial='".$_GET['orderid']."'");
		...
		echo $imagepaid;
	}else if ($sqlrow['paystatus'] == 'cancel') {
