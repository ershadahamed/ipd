...
function user_not_fully_set_up($user) {
    ...
    // return (empty($user->firstname) or empty($user->lastname) or empty($user->email) or over_bounce_threshold($user));  // modi by arizan abdullah
   //if($user->usertype=='Active candidate'){
   if($user->id!='2'){
	if($user->usertype=='Active candidate'){
		return (empty($user->firstname) or empty($user->lastname) or empty($user->email) or empty($user->college_edu) or ($user->highesteducation=='0') or empty($user->yearcomplete_edu) or over_bounce_threshold($user));
	}else{
		return (empty($user->firstname) or empty($user->lastname) or empty($user->email) or empty($user->department) or empty($user->designation));
	}	
	}
}
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml='', $attachment='', $attachname='', $usetrueaddress=true, $replyto='', $replytoname='', $wordwrapwidth=79) {
    ...
    if (!validate_email($user->email)) {//echo "====".$user->email;?><script>alert("valid");</script><?php
        ...
        return false;
    }else{//echo "====".$user->email;?><script>alert("valid");</script><?php
}
