...
class global_navigation extends navigation_node {
    ...
    public function initialise() {
       ...
       // $this->rootnodes['mycourses'] = $this->add(get_string('mycourses'), null, self::TYPE_ROOTNODE, null, 'mycourses');
		$this->rootnodes['mycourses'] = $this->add(get_string('courses'), null, self::TYPE_ROOTNODE, null, 'courses');
        ...
        if (count($mycourses) > 0) {
            ...
            foreach ($mycourses as $course) {
				//if($course->category=='1'){ //edited by arizan 23april2013// to hide category except course category
					$course->coursenode = $this->add_course($course, false, true);
				//}
            }
        }
		
		//added by arizan 20120902
		$queryrole  = $DB->get_records('role_assignments',array('userid'=>$USER->id));
		foreach($queryrole as $qrole){ }
        if(($USER->id =='2') || ($qrole->roleid =='10')){
		if ($showallcourses) {
            // Load all courses
    ...
    protected function load_all_courses($categoryids=null) {
		...
		//edited by arizan 20121002//c.category!='6'
        ...
        $sql = "SELECT c.id, c.sortorder, c.visible, c.fullname, c.shortname, c.category, cat.path AS categorypath $ccselect
             ...
             LEFT JOIN {course_categories} cat ON cat.id=c.category
                 WHERE c.category!='6' AND c.id {$courseids} {$categoryselect}
...
class settings_navigation extends navigation_node {
    ...
    protected function load_course_settings($forceopen = false) {
		...
		//$se=mysql_query("SELECT * FROM mdl_cifacourse_categories WHERE id='".$course->category."'");
		//$sq=mysql_fetch_array($se);
		if($course->category == '3'){
			$coursenode = $this->add(get_string('examadministration'), null, self::TYPE_COURSE, null, 'courseadmin');		
		}else{
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle='usercurrentsettings') {
        ...
        if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if (($currentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('moodle/user:update', $systemcontext)) {
                ...
                if ($userauthplugin && $userauthplugin->can_edit_profile()) {
					...
					if(($qrole->roleid=='9') || ($qrole->roleid=='14')){
						$usersetting->add(get_string('viewprofile'), $url, self::TYPE_SETTING);
					}else{
						//$usersetting->add(get_string('editmyprofile'), $url, self::TYPE_SETTING); //remove arizan
					}
                }
            }
        }
        ...
        // Messaging
        //edited by arizanabdullah 22052012
