...
function get_course_mods($courseid) {
    ...
    return $DB->get_records_sql("SELECT cm.*, m.name as modname
                                  ...
                                  WHERE cm.course = ? AND cm.module = m.id AND m.visible = 1", array($courseid)); // no disabled mods
}
...
function updatecourse_subscription($userid) {
    global $DB;

    $sqltext = "Select
                    *, a.idnumber as coursecode, 
                    FROM_UNIXTIME(d.firstaccess,'%Y-%m-%d') as subscriptionstart,
                    DATE_ADD(FROM_UNIXTIME(d.firstaccess,'%Y-%m-%d'), INTERVAL 60 DAY) as enddate,
                    DATE_ADD(FROM_UNIXTIME(d.lastaccess,'%Y-%m-%d'), INTERVAL 60 DAY) as subscriptionend,
					DATE_ADD(FROM_UNIXTIME(c.timestart,'%Y-%m-%d'), INTERVAL 60 DAY) as subscriptionend2
                From
                ...
                Where
                    ...
                    a.visible='1' And c.ustatus='0' And
                    c.userid = '" . $userid . "'"; 
    $sql = $DB->get_recordset_sql($sqltext);
    $no = '1';
    foreach ($sql as $data) {
        $duration = ($data->duration) * 30;       // course duration
        $timestart = $data->firstaccess;
        $subscriptionend = strtotime($data->subscriptionstart . "+ " . $duration . " days");
		$subscriptionend2 = strtotime($data->subscriptionend2);

       /*mmn off to add enddate based on subscription date $updatesql = "UPDATE {user_enrolments} 
		SET timestart='" . $timestart . "', timeend='" . $subscriptionend . "', ustatus='1'
		WHERE enrolid='" . $data->enrolid . "' AND userid='" . $userid . "'
		";*/
		$updatesql = "UPDATE {user_enrolments} 
		SET timeend='" . $subscriptionend2 . "', ustatus='1'
		WHERE enrolid='" . $data->enrolid . "' AND userid='" . $userid . "'
		";
...
function get_rolename_candidate() {
    global $DB;
...
function printRoleAssignmentsId($userid, $contextid) {
    global $DB;

    $sql = "SELECT * FROM {role_assignments} WHERE contextid='" . $contextid . "' AND userid='" . $userid . "'";
    $getroleid = $DB->get_record_sql($sql);
    return $getroleid->roleid;
}

/*
 * Get country code for users
 */

function getUserCountrycode($countrycode) {
    global $DB;

    $sql = "SELECT * FROM {country_list} WHERE countrycode='" . $countrycode . "'";
    $getcode = $DB->get_record_sql($sql);
    return $getcode->iso_countrycode;
}
...
function get_grouplist() {
    ...
    $sql = "    Select
		...
		From
		  {groupings} a
";
    $getgroup = $DB->get_recordset_sql($sql);
    return $getgroup;
}

function grouplist() {
    global $DB;
    $field = "a.id, a.courseid, a.name";
    $sql = "Select {$field} From {groupings} a";
    $getgroup = $DB->get_record_sql($sql);
    return $getgroup;
}

function get_subgrouplist($groupid) {
    global $DB;
    ...
    $sqltext = "Select
        ...
        b.groupid,
        a.name,
        c.name As subgroupname,
        c.description,
        d.userid
      From
        {groupings} a Inner Join
        {groupings_groups} b On a.id = b.groupingid Inner Join
        {groups} c On b.groupid = c.id Inner Join
        {groups_members} d On c.id = d.groupid
...
function subgrouplist($groupingid) {
    global $DB;
    ...
    $sqltext = "Select
      ...
      From
        {groupings} a Inner Join
        {groupings_groups} b On a.id = b.groupingid Inner Join
        {groups} c On b.groupid = c.id
      Where b.groupingid='" . $groupingid . "'
  ";
    $sql = $DB->get_recordset_sql($sqltext);
    return $sql;
}

function listoutall_groupusers($userid) {
    global $DB;
    ...
    $sql = "Select
        ...
        d.userid
      From
        mdl_cifagroupings a Inner Join
        mdl_cifagroupings_groups b On a.id = b.groupingid Inner Join
        mdl_cifagroups c On b.groupid = c.id Inner Join
        mdl_cifagroups_members d On c.id = d.groupid
      Where
        d.userid='" . $userid . "'
  ";
...
/*
 * List out all users with userid as parameter
 */

function get_user_details($userid) {
    global $DB;
    $sqltext = "Select * From {user} Where id='" . $userid . "'";
    return $DB->get_record_sql($sqltext);
}
...
function get_reportdetails($reportid) {
    global $DB;
 ...
 * Created foreach loop to get the list
 */

function get_coursedetails($userid) {
    global $DB;

    $sqltext = "Select
                a.courseid, b.enrolid, b.timestart, b.timeend, b.timecreated as enrollmenttime,
                c.fullname, c.idnumber,
                a.cost, a.currency, d.traineeid, d.firstname, d.lastname, d.timecreated as registrationdate
              From
                {enrol} a Inner Join
                {user_enrolments} b On a.id = b.enrolid Inner Join
                {course} c On a.courseid = c.id Inner Join
                {user} d On b.userid = d.id
              Where
                b.userid = '" . $userid . "' And
                a.enrol = 'manual' And
                c.category = '1' And a.roleid='5'
  ";
    return $DB->get_recordset_sql($sqltext);
}

function generatecadidateid($country) {
    global $DB;
    // total user
    $DB->count_records('user', array('suspended' => '0'));

    $sql = $DB->get_record_sql("SELECT * FROM {user} WHERE suspended='0' ORDER BY id DESC");
    $tid = $sql->id + 1;
    $year = date('y');

    if ($sql->id <= '9') {
        $c = '00';
    } elseif ($sql->id >= '10' && $sql->id <= '99') {
        $c = '0';
    } else {
        $c = '';
    }
...
function generatecadidateid2($country) {
    global $DB;
    // total user
    $DB->count_records('user', array('suspended' => '0'));
    ...
    $candidateID = $year . '' . $c . '' . $tid . '' . $country;
    return $candidateID;
}

/*
 * Display all selected user for schedule
 */

function selecteduserforschedule($scdid, $userid, $reportid) {
    global $DB;
    ...
    $sql = "		Select
		...
		Where
		  b.scheduling_id = '" . $scdid . "' And b.recipient_id='" . $userid . "' And a.rid='" . $reportid . "'";
    return $DB->get_record_sql($sql);
}

function percentage($userid, $scoid) {
    global $DB;

    $per = $DB->get_record('scorm_scoes_track', array('userid' => $userid, 'scoid' => $scoid, 'element' => 'cmi.core.total_time'));
    return $per->timemodified;
}

function getScormId($courseid, $instancename) {
    global $DB;

    $sql = "SELECT * FROM {scorm} WHERE scormtype ='local' AND course = '" . $courseid . "' AND name = '" . $instancename . "'";
    $rs = $DB->get_record_sql($sql);
    return $rs;
}

/*
 * Count total scorm in a course
 */

function getTotalScormOnCourse($courseid) {
    global $DB;

    $sql = "SELECT COUNT(a.id) FROM {scorm} a WHERE a.course = '" . $courseid . "'";
    $rs = $DB->count_records_sql($sql);
    return $rs;
}

function chapteronscorm($scormid) {
    global $DB;

    $sql = "Select COUNT(a.id)
      From
        {scorm} a Inner Join
        {scorm_scoes} b On a.id = b.scorm
      Where
        a.id = '" . $scormid . "' And
        b.scormtype = 'sco'
    ";
...
function attemptscormpercent($userid, $scormid) {
    global $DB;

    $count = $DB->count_records('scorm_scoes_track', array('userid' => $userid, 'scormid' => $scormid, 'element' => 'cmi.core.lesson_status', 'value' => 'completed'));
    $inprogress = 'In progress';
    $completed = 'Done';

    if ($count > '0') { // completed status
        /*mmn off previous calculation
	   $p = round((100 / chapteronscorm($scormid)) * $count);
        if ($p == '100') {
            $percent = $completed . ' (' . $p . '%)';
        } else {
            $percent = $inprogress . ' (' . $p . '%)';
        }*/
 $percent = $completed . ' (100%)';

    } else {    // incompleted status
       /*mmn off previous calculation
	 $percent = $inprogress . ' (0%)';*/
$count2 = $DB->count_records('scorm_scoes_track', array('userid' => $userid, 'scormid' => $scormid, 'element' => 'cmi.core.lesson_status', 'value' => 'incomplete'));
if($count2>1){
$p = round((100 / chapteronscorm($scormid))*$count2);
}else{
$p = 0;
}
$percent = $inprogress . ' (' . $p . '%)';
...
function printScormWithCourseId($courseid) {
    global $DB;

    // display all chapter in course by scormid
    $sql = "Select   a.course,
                b.scorm,
                a.name,
                b.title, b.id as scoid
          From
            {scorm} a Inner Join
            {scorm_scoes} b On a.id = b.scorm
          Where
            a.course = '" . $courseid . "' And
            b.scormtype = 'sco' Group By b.scorm";
    $rs = $DB->get_recordset_sql($sql);
    return $rs;
}

/*
 * Enrol id for courses
 */

function get_coursesenrolid($courseid) {
    global $DB;

    $sql = "SELECT id FROM mdl_cifaenrol WHERE courseid = '" . $courseid . "'";
...
function coursedetails() {
    ...
    $sql = "SELECT * FROM {course} WHERE category='1' Order By idnumber ASC";
    $data = $DB->get_recordset_sql($sql);
    return $data;
}

function listout_userswithstatus($enrolid, $rolename, $fromrecords, $perpage, $search = '') {
    global $DB;

    $qry = "SELECT b.lasttimecreated, a.enrolid, b.lastaccess, b.firstaccess, b.firstname, b.lastname, b.traineeid,a.userid,a.timestart,a.timeend,b.lastlogin, 
			FROM_UNIXTIME(b.firstaccess,'%d/%m/%Y') as startdate, 
			FROM_UNIXTIME(a.timestart,'%d/%m/%Y') as timecreated, 
                        FROM_UNIXTIME(a.timeend,'%d/%m/%Y') as expirydate,
                        FROM_UNIXTIME(b.lastaccess,'%d/%m/%Y') as lastuseraccess, 
    ...
    $qry .= "           WHERE a.enrolid='" . $enrolid . "' AND (b.usertype='" . $rolename . "' OR b.usertype='Inactive candidate') AND b.deleted!='1' AND a.userid=b.id";
    if ($search != '') {
        // $qry .= "       AND FROM_UNIXTIME(b.lasttimecreated,'%m') =  $search";
        $qry .= "       AND (date_format(from_unixtime(a.timeend), '%m') LIKE '%" . $search . "')";
        $qry .= "       ORDER BY b.timecreated DESC, a.userid DESC";
    } else {
        $qry .= "           ORDER BY b.timecreated DESC, a.userid DESC LIMIT $perpage OFFSET $fromrecords";
    }
    return $sql = $DB->get_recordset_sql($qry);
}

function count_listout_userswithstatus($enrolid, $rolename) {
    global $DB;

    $sql = "SELECT COUNT(DISTINCT a.userid)  
			FROM mdl_cifauser_enrolments a, mdl_cifauser b ";
    $sql .= "		WHERE a.enrolid='" . $enrolid . "' AND b.usertype='" . $rolename . "' AND b.deleted!='1' AND a.userid=b.id";
    $sql .= "		ORDER BY b.timecreated DESC";
    return $DB->count_records_sql($sql);
}

/*
 * List out HQ organization name
 */

function getOrganization($orgid) {
    global $DB;

    $sql = "SELECT id, groupofinstitution, name FROM {organization_type} WHERE id = '" . $orgid . "'";
    $data = $DB->get_record_sql($sql);
    return $data->name;
}

function getOrganizationId($orgid) {
    global $DB;

    $sql = "SELECT id, groupofinstitution, name, organizationid, email FROM {organization_type} WHERE id = '" . $orgid . "'";
    $data = $DB->get_record_sql($sql);
    return $data;
}

function getOrganizationRoleID($orgid) {
    global $DB;

    $sql = "SELECT id, groupofinstitution, name, org_type FROM {organization_type} WHERE id = '" . $orgid . "'";
    $data = $DB->get_record_sql($sql);
    return $data->org_type;
}

/*
 * Get course id with selected category
 */

function getCourseIDWithCategory($category, $visible) {
    global $DB;

    $sql = "SELECT id, category, fullname FROM {course} WHERE category = '" . $category . "' AND visible='" . $visible . "'";
    $data = $DB->get_recordset_sql($sql);
    return $data;
}

/*
 * Insert new organization records to Grouping table DB
 */

function insertNewGrouping($courseid, $HQorgname) {
    global $DB;

    $record = new stdClass();
    $record->courseid = $courseid;
    $record->name = $HQorgname;
    $record->description = $HQorgname;
    $record->descriptionformat = 1;
    $record->timecreated = time();
    $record->timemodified = time();
    $newOrgName = $DB->insert_record('groupings', $record, false);
    return $newOrgName;
}

function count_orgtitle($cid) {
    global $DB;

    $sql = "Select COUNT(DISTINCT id) From {organization_config} Where organizationtitle='" . $cid . "'";
    return $DB->count_records_sql($sql);
}

function listorgtitle_withtitleid($cid) {
    global $DB;

    $sqlc = "Select * From {organization_config} Where organizationtitle='" . $cid . "'";
    $rs = $DB->get_recordset_sql($sqlc);

    return $rs;
}

/// reset user registartion
function resetUserRegistration($userid, $duration) {
    global $DB;

    //184 //120 day
    $today = time();
    $lasttimecreated = strtotime("+ " . $duration . " day", time());

    $sqlc = "UPDATE {user} SET usertype='Active candidate', firstaccess='" . $today . "', timecreated='" . $today . "', lasttimecreated='" . $lasttimecreated . "' WHERE id='" . $userid . "'";
    $rs = $DB->execute($sqlc);

    return $rs;
}

function getThisUserInfo($userid) {
    global $DB;

    $sql = "Select firstname, lastname, password, traineeid,lasttimecreated From {user} Where id='" . $userid . "'";
    $rs = $DB->get_record_sql($sql);

    return $rs;
}

function updateThisUserPassword($userid, $duration, $password) {
    global $DB;
    $lasttimecreated = strtotime("+ " . $duration . " day", time());

    $sql = "UPDATE {user} SET password='" . $password . "', lasttimecreated='" . $lasttimecreated . "' Where id='" . $userid . "'";
    $rs = $DB->execute($sql);

    return $rs;
}

function viewScormDetails($courseid) {
    global $DB;

    $sql = "Select * From {scorm} Where course='" . $courseid . "'";
    $rs = $DB->get_recordset_sql($sql);

    return $rs;
}

function updateScormColumn($scormid, $courseid) {
    global $DB;

    $sql = "UPDATE {scorm} SET displayattemptstatus='1', skipview='2', hidetoc='0', auto='1' Where id='" . $scormid . "' And course='" . $courseid . "'";
    $rs = $DB->execute($sql);
...
function communicationPreference() {
    global $DB;
    $sql = "Select * From {communication_rules} Where status='0' Order By id Asc";
    $rs = $DB->get_recordset_sql($sql);

    return $rs;
}

function getListOfloggedinUsers($month) {
    global $DB;

    $sql = "Select
    a.action,
    From_UnixTime(a.time, '%d/%m/%Y') As logdate
    From
      mdl_cifalog a,
      mdl_cifauser b
    Where
      ...
      (a.action = 'login' And
      a.userid <> '2' And
      From_UnixTime(a.time, '%m') = '" . $month . "') Group By From_UnixTime(a.time, '%d/%m/%Y')";
    $rs = $DB->get_recordset_sql($sql);

    return $rs;
}

//get total loggedin by date
function totalLoggedinByDate($reportdate) {
    global $DB;

    $sql = "Select COUNT(DISTINCT a.time), From_UnixTime(a.time, '%d/%m/%Y') As logdate
    From
      mdl_cifalog a,
      mdl_cifauser b
    Where
      ...
      (a.action = 'login' And
      a.userid <> '2' And
      From_UnixTime(a.time, '%d/%m/%Y') = '" . $reportdate . "')";
    return $DB->count_records_sql($sql);
}

function listSubscribeBydate($month, $courseid) {
    global $DB;

    $sql = "
Select From_UnixTime(b.timemodified, '%d/%m/%Y') as subscribedate
From
  mdl_cifauser a,
  mdl_cifarole_assignments b,
  mdl_cifacontext c,
  mdl_cifacourse d
Where
  a.id = b.userid And
  a.deleted!='1' And
  b.contextid = c.id And
  c.instanceid = d.id And
  (a.id <> '2' And
  d.id = '" . $courseid . "' And
  From_UnixTime(b.timemodified, '%m') Like '$month%' And
  c.contextlevel = '50') Group By subscribedate";

    return $DB->get_recordset_sql($sql);
}

//
function totalSubscribeBydate($reportdate, $courseid) {
    global $DB;

    $sql = "
Select COUNT(DISTINCT a.timemodified)
From
  mdl_cifauser a,
  mdl_cifarole_assignments b,
  mdl_cifacontext c,
...
Where
  ...
  (a.id <> '2' And
  d.id = '" . $courseid . "' And
  From_UnixTime(b.timemodified, '%d/%m/%Y') Like '$reportdate%' And
  c.contextlevel = '50')";

    return $DB->count_records_sql($sql);
}

function listSubscribeByYear($courseid, $grouptype) {
    global $DB;

    $sql = "
 Select
  From_UnixTime(c.timecreated, '%Y') as subscribedate
From
...
Where
  a.orgtype = '" . $grouptype . "' And
  b.courseid = '" . $courseid . "' And
  a.deleted!='1' And
  a.id <> '2' And
  (a.usertype = 'Active candidate' Or
    a.usertype = 'Inactive candidate') Group By subscribedate";

    /* $sql = "
      Select From_UnixTime(b.timemodified, '%Y') as subscribedate
      From
      mdl_cifauser a,
      mdl_cifarole_assignments b,
      mdl_cifacontext c,
      ...
      c.instanceid = d.id And
      (a.id <> '2' And
      d.id = '" . $courseid . "' And (a.usertype = 'Active candidate' Or a.usertype = 'Inactive candidate') And
      c.contextlevel = '50') Group By subscribedate"; */

    return $DB->get_recordset_sql($sql);
}

function totalSubscribeByYear($year, $courseid, $grouptype) {
    global $DB;
    ...
    $sql = "    
        ...
        From
          ...
          mdl_cifacontext e On e.instanceid = b.courseid
        Where
          a.suborgtype = '" . $grouptype . "' And
          b.courseid = '" . $courseid . "' And
          a.deleted!='1' And
          a.id <> '2' And
          (a.usertype = 'Active candidate' Or
            a.usertype = 'Inactive candidate') And  From_UnixTime(c.timecreated, '%Y') = '" . $year . "'";

    return $DB->count_records_sql($sql);
}

// total user subscribed by organization
function totalUsersSubscribeByOrg($grouptype) {
    global $DB;
    ...
    $sql = "    
        ...
        From
            mdl_cifauser a Inner Join
            mdl_cifaorganization_type d On a.suborgtype = d.id
        Where
          a.suborgtype = '" . $grouptype . "' And
          a.deleted!='1' And
          a.id <> '2' And
...
// total course subscribed by organization
function totalCourseSubscribeByOrg($courseid, $grouptype) {
    global $DB;
...
function getListOfActiveUsers($month) {
    global $DB;

    $sql = "Select
    a.action,
    b.traineeid, b.firstname, b.lastname,
    From_UnixTime(a.time, '%d/%m/%Y') As logdate
    From
      mdl_cifalog a,
      mdl_cifauser b
    Where
      a.userid = b.id And b.deleted='0' And
      (a.action = 'login' And
      a.userid <> '2' And
      From_UnixTime(a.time, '%m') = '" . $month . "') And "
            . "(b.id!='132' And b.id!='133' And b.id!='134' And b.id!='136' And b.id!='313')"
            . "Group By b.traineeid";
    $rs = $DB->get_recordset_sql($sql);

    return $rs;
}

function getListEnrolCourse($code) {
    global $DB;
    ...
    $sql = "	Select
		...
		Where
		  ...
		  b.enrol = 'manual' And
		  a.visible = '1' And
		  b.status = '0' And a.idnumber='" . $code . "'";
    return $DB->get_recordset_sql($sql);
}

function totalEnrolCourse($userid, $enrolid) {
    global $DB;

    $sql = "Select COUNT(DISTINCT userid) FROM {user_enrolments} WHERE userid='" . $userid . "' AND enrolid='" . $enrolid . "'";
    return $DB->count_records_sql($sql);
}

function statusCoursePayment($courseid, $traineeid) {
    global $DB;

    $statement = " {candidates} a Inner Join {orders} b On a.serial = b.customerid Inner Join {order_detail} c On b.serial = c.orderid";
    $sqlcourse2 = "SELECT * FROM {$statement} WHERE c.productid='" . $courseid . "' AND a.traineeid = '" . $traineeid . "'";
    $sqlcourse2.=" Order By b.date DESC, a.traineeid DESC";

    return $DB->get_record_sql($sqlcourse2);
}

// GET COURSE CONTEXT ID
function getContextIdCourse($courseid) {
    global $DB;

    $sql = "Select * From {context} Where contextlevel='50' And instanceid='" . $courseid . "'";
    $rs = $DB->get_record_sql($sql);
    return $rs;
}

/*SELECT DISTINCT a.userid, b.firstname
FROM mdl_cifalog a, mdl_user b
WHERE a.action = 'login'
AND a.userid <> '2'
AND From_UnixTime(a.time, '%m') = '" . $month . "'
AND a.userid = b.id
AND b.deleted=0*/
