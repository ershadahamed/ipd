...
function updatecourse_subscription($userid) {
    ...
    $sqltext = "Select
                    *, a.idnumber as coursecode,
                    FROM_UNIXTIME(d.firstaccess,'%M %d, %Y') as subscriptionstart,
    ...
    foreach ($sql as $data) {
        //if ($no > 1) {
        //$timestart = $data->lastaccess;
        //$subscriptionend = strtotime($data->subscriptionend);
        //} elseif($no <='1') {
        // if 1 course
        $timestart = $data->firstaccess;
        $subscriptionend = strtotime($data->enddate);
        //}
...
function get_subgrouplist($groupingid) {
    global $DB;
...
function attemptscormpercent($userid, $scoid) {
    global $DB;
    $timenow = strtotime('now');
    $count = $DB->count_records('scorm_scoes_track', array('userid' => $userid, 'scoid' => $scoid, 'element' => 'x.start.time'));
    $inprogress = 'In progress';
    $completed = 'Done';
    if ($count) {
        $sql = "Select * From {scorm_scoes_track} "
                . "Where userid='" . $userid . "' And scoid='" . $scoid . "' And (element='x.start.time' Or element='cmi.core.lesson_status')";
        $per = $DB->get_recordset_sql($sql);
        foreach ($per as $perc) {
            $timestart = $perc->value;
            $gettime = $timenow - $timestart;
            $maxtime = '30';
            if (format_time($gettime) <= $maxtime) {
                $percent = $inprogress . ' (20%)';
            } elseif (format_time($gettime) <= $maxtime) {
                $percent = $inprogress . ' (55%)';
            } elseif (format_time($gettime) <= $maxtime) {
                $percent = $inprogress . ' (70%)';
            } elseif (format_time($gettime) <= $maxtime) {
                $percent = $inprogress . ' (95%)';
            } elseif ($perc->value == 'completed') {
                $percent = $completed . ' (100%)';
            } elseif ($perc->value == 'incomplete') {
                $percent = $inprogress . ' (25%)';
            } else {
                $percent = $inprogress . ' (73%)';
            }
        }
    } else {
...
function listout_userswithstatus($enrolid, $rolename, $fromrecords, $perpage, $search = '') {
    ...
    $qry = "SELECT a.enrolid, b.lastaccess, b.firstaccess, b.firstname, b.lastname, b.traineeid,a.userid,a.timestart,a.timeend,b.lastlogin, 
    ...
    $qry .= "           WHERE a.enrolid='" . $enrolid . "' AND b.usertype='" . $rolename . "' AND b.deleted!='1' AND a.userid=b.id";
    if($search!=''){
        $qry .= "       AND FROM_UNIXTIME(b.timecreated,'%m') =  $search";
