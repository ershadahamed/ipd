...
// auto create users to report (if selected report is GROUP)
if ($DB->count_records('report_menu', array('id' => $_GET['id'], 'usergroup' => '2'))) {
    // get a role name
    $conditions = array('id' => '5');
    $qb = $DB->get_record('role', $conditions, $fields = 'name', $strictness = IGNORE_MISSING);
    $rolename = $qb->name;
    // get list of candidate with organization type=7(ADIB)
    $groupusers = $DB->get_recordset_sql("SELECT *, c.timestart as enroltime, a.id as examid
                                                  FROM mdl_cifacourse a Inner Join mdl_cifaenrol b On a.id = b.courseid Inner Join
                                                    mdl_cifauser_enrolments c On b.id = c.enrolid Inner Join
                                                    mdl_cifauser d On c.userid = d.id                                                
                                                  WHERE a.category = '9' AND d.usertype='" . $rolename . "' 
                                                    AND d.usertype = 'Active candidate' And d.orgtype = '7'");
    foreach ($groupusers as $group) {
        // check if user not in list
        $crs = $DB->count_records('report_users', array('reportid' => $_GET['id'], 'candidateid' => $group->userid));
        if ($crs != '1') {
            // Adding users to report_users TABLE
            $record = new stdClass();
            $record->reportid = $reportid;
            $record->usergroup = '2';
            $record->candidateid = $group->userid;
            $record->firstname = $group->firstname;
            $record->lastname = $group->lastname;
            $record->dob = $group->dob;
            $record->organization = $group->orgtype;
            $lastinsertid = $DB->insert_record('report_users', $record, false);
        }
    }
    $groupusers->close();
}
// END auto create users to report (if selected report is GROUP)
...
$reportQRY = "
		...
		Where
			b.reportid='" . $reportid . "'	
	";
//mmn add to view active user only
$reportQRY .= "AND c.status=0";
$reportSQL = mysql_query($reportQRY);
...
if ($viewreport['selectedreport'] == '0') {
    ...
    <div style="overflow-x:scroll;width:100%;">
        <fieldset style="padding: 1em;" id="user" class="clearfix">
            ...
            <form name="form1" id="form1" action="<?= $CFG->wwwroot . '/examcenter/download_reportcsv.php?rid=' . $reportid . '&rcid=' . $viewreport['reportcreator'] . '&excel=excel&html=html&sid=' . $_GET['sid']; ?>" method="post">
                ...
                <table style="padding:0px; border: 0px solid #666666; border-collapse:collapse;margin:0px auto;" width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        <td align="left">
                            <!--b>DOWNLOAD</b-->
                            <input type="submit" name="download_html" id="id_defaultbutton" value="HTML" title="<?= get_string('titledownload'); ?>" onClick="this.form.action = '<?= $CFG->wwwroot . '/examcenter/download_reporthtml.php?rid=' . $reportid . '&rcid=' . $viewreport['reportcreator'] . '&excel=excel&html=html&sid=' . $_GET['sid']; ?>', target = '_blank'" />
                            <input type="submit" name="download_excel" id="id_defaultbutton" value="EXCEL" title="<?= get_string('titledownload'); ?>" onClick="this.form.action = '<?= $CFG->wwwroot . '/examcenter/download_reportexcel.php?rid=' . $reportid . '&rcid=' . $viewreport['reportcreator'] . '&excel=excel&html=html&sid=' . $_GET['sid']; ?>'" />
	...
	";
				...
				$statement .= "AND d.status=0";
                if ($USER->id != '2') {
                    $statement.=" AND e.orgtype='" . $USER->orgtype . "'";
                }
                $csql = "SELECT *, e.id as userid, c.id as enrollmentid FROM {$statement}";
                $sqlquery = mysql_query($csql);
                // echo $c=mysql_num_rows($sqlquery);
                ?>		
                <table width="95%" border="1" id="searchtable" style="margin:0px auto;">
                    <tr style="background-color:#ccc;">
                        <?php if (($creator['id'] == '12') || ($creator['id'] == '13')) { ?>
                            <?php if ($viewreport['organizationname'] != '0') { ?>
                                <th width="15%" scope="row"><?= get_string('organization'); ?></th>
                            <?php }if ($viewreport['employeeid'] != '0') { ?>
                                <th width="15%" scope="row"><?= get_string('employeeid'); ?></th>			  
                    ...
                    if ($mycount != '0') {
                        while ($sqlrow = mysql_fetch_array($sqlquery)) {
                            $bil = $no++;
//mmn add to update expirydate
if($sqlrow['userid']!=''){
updatecourse_subscription($sqlrow['userid']);
 $statement2 = "
	  mdl_cifacourse a Inner Join
	  mdl_cifaenrol b On a.id = b.courseid Inner Join
	  mdl_cifauser_enrolments c On b.id = c.enrolid Inner Join
	...
	";
                ...
                $statement2.=" WHERE a.category='1' AND d.reportid = '" . $reportid . "' And a.visible != '0'";
				$statement2 .= "AND d.status=0 AND e.id='".$sqlrow['userid']."'";
                if ($USER->id != '2') {
                    $statement2.=" AND e.orgtype='" . $USER->orgtype . "'";
                }
                $csql2 = "SELECT *, c.id as enrollmentid FROM {$statement2}";
                $sqlquery2 = mysql_query($csql2);
				$sqlrow2 = mysql_fetch_array($sqlquery2);
}
//mmn end update expiry date
               ...
               $a++;
                                ...
                                <?php } if ($viewreport['expirydate'] != '0') { ?>
                                    <td style="text-align:center;">
                                        <?php
                                        //if ($sqlrow['firstaccess'] != 0) {
                                            echo date('d-m-Y', $sqlrow2['timeend']);
                                        /*} else {
                                            echo ' - ';
                                        }*/
                                        ?>
...
<?php } else { ?>
	...
	";
		...
		Where
		  a.category = '1' And
		  d.reportid = '" . $sqlrow['reportid'] . "' And d.status=0 And
	...
	");
                        ...
                        if ($sqlrow['firstaccess']) {
                            $totaltimeaccess = format_time(time() - $sqlrow['firstaccess']);
                        } else {
                            $totaltimeaccess = '0';
                        }
	...
	");
				...
				";
					...
					Where
					  d.grade >= '60' And
					  a.visible != '0' And
					  e.reportid = '" . $sqlrow['reportid'] . "' And e.status=0 And
