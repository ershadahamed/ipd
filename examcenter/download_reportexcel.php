...
if ($checkBox != '') {
    ...
    if ($sid != $srid[2]) {
        ...
        if ($gns->enrolstatus) {
            $headers[] = get_string('enrolstatus');
            $profileheaders[] = $c;
        }        
    }
    if ($sid == $srid[0]) {
        ...
        if ($gns->examinationstatus) {
            $headers[] = get_string('teststatus')." 1";
			$headers[] = "Marks 1 (%)";
			$headers[] = "Date Attempt 1";
			$headers[] = get_string('teststatus')." 2";
			$headers[] = "Marks 2 (%)";
			$headers[] = "Date Attempt 2";
        }
...
";
        ...
        if ($sid == $srid[0]) {
            ...
            $sql = "SELECT *, c.id as enrollmentid, a.idnumber as coursecode, c.lastaccess as courselastaccess  FROM {$statement}";
        }
        ...
        foreach ($results as $data) {
            ...
            //if ($data->enrolstatus!=0) {
                $rsenrolstatus = checkuserstatus($data->userid);
            //} else {
                //$rsenrolstatus = " - ".$data->enrolstatus. checkuserstatus($data->userid);
            //}  // Enrol Status Here            
		...
		";
				...
				if($testgrade1['timefinish']=='0'){$datefinish1 = "incomplete";}else{$datefinish1 = date('d/m/Y H:i:s', $testgrade1['timefinish']);}
               ...
               /*//mmn off to display subscriptiondate } else {
                ...
                // courselastaccess access
                if ($data->firstaccess!=0) {
					if($data->courselastaccess!=0){
                    $courselastaccess = date('d/m/Y', $data->courselastaccess);
					}else{ $courselastaccess = ' - '; }
                } else {
                    $courselastaccess = ' - ';
                }
            } else if ($sid == $srid[1]) {
            ...
            if ($gns->enrolstatus) {
                $row[] = ucwords(strtolower($rsenrolstatus));
            }            
            ...
            if ($sid == $srid[0]) {
                ...
                /*mmn off if ($gns->markperformance) {
                    $row[] = $fgrade;
                }*/
                if ($gns->subscriptiondate) {
