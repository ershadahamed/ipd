...
?>
		<tr>
		  ...
		  <td style="text-align:center">(<?=round($slist['grade']);?>%)</td>
		  <td>&nbsp;</td>
		  <td style="text-align:center;">
		  <?php
			/* $a = new DateTime(date('H:m:s', $slist['timefinish']));
			$b = new DateTime(date('H:m:s', $slist['timestart']));
			$interval = $a->diff($b);

			echo $interval->format("%H:%m:%s"); */
			echo format_time($slist['timefinish'] - $slist['timestart']);
			//$workingHours = ((($slist['timefinish']) - ($slist['timestart'])) / 3600);
			//echo date('H:m:s', $workingHours);
			//echo $slist['timefinish'];
		  ?>		  
		  <?//=date('H:m:s', $slist['timefinish']);?>
		  </td>
