...
<?php
	...
	// $nrole=mysql_fetch_array($role);	
        ...
        $dbtable = 'role'; ///name of table
        $conditions = array('id'=>'5', ); ///the name of the field (key) and the desired value
        $sort = 'id'; //field or fields you want to sort the result by
        $fields = 'name'; ///list of fields to return
        $result = $DB->get_records($dbtable,$conditions,$sort,$fields);

        foreach($result as $re){ $nrole=$re->name;}        
	//echo $nrole;
...
?>
		<tr><td colspan="10" scope="row"><?=get_string('recordsnotfound');?></td></tr>
<?php } ?>
		</table><br/>	  
  </fieldset>
  </div>
...
<fieldset style="padding: 1em;" id="user" class="clearfix">
<legend style="font-weight:bolder;" class="ftoggler"><?=get_string('orgtype');?></legend>
...
<?php
foreach (get_grouplist() as $data) {
    ?>
                                {
                                id: 0, text: "<?= $data->name; ?>", title:"sadsadasdasda", expanded: false, spriteCssClass: "folder", items: [
    <?php
    foreach (subgrouplist($data->id) as $groupdata) {
        ?>
                                    { id: <?= $groupdata->groupid; ?>, text: "<?= $groupdata->subgroupname; ?>", title:"sadsadasdasda", spriteCssClass: "html" },
    <?php } ?>
                                ]
                                },<?php } ?>
                            ]
