  ...
  <tr style="<?php if ($pstatus1){ echo $color1; }else if ($pstatus2){ echo $color2; }else{ echo $color3; } ?>">
	...
	<?php 
		...
		$cfinaltestsql=mysql_query("
			...
			Where
			  e.userid='".$sqlrow['candidateid']."'	And b.course='".$suser['productid']."' And c.id='".$sqlrow['resitid']."'	
		");
		$final=mysql_fetch_array($cfinaltestsql); 
		$finalexamid=$final['cifaquizid'];
		
		if($qselcourse['category']=='3') // exam // re-sit
		{		
			echo $examname=$final['name'].' - ('.get_string('resitexam').')';		
		}else{ 
			if($sqlrow['resitid']=='3'){ 
				echo $qselcourse['fullname'].' - (Extended)';  // extension
			}else{
				echo $b++.') - '.$qselcourse['fullname'];
			}
		}
		echo '<br/>';
		}
		...
		<?php
			...
			if ($sqlrow['paystatus'] == $pendingstatus) {
				if($sqlrow['paymethod'] != 'telegraphic'){
					...
					// email to commenter
						$to = $sqlrow['email'];
						$subject = "Reminder: ".strtoupper($sqlrow['traineeid'])." - Enrollment Not Complete";
					...
					if($today > $expiry){
						...
						// email to commenter
						$to = $sqlrow['email'];
						$subject = "Next Step:  ".strtoupper($sqlrow['traineeid'])." - Candidate Enrollment Cancellation";
						...
						if($qselcourse['category']=='3'){ 
						?>
						<input title="<?=$confirmedpay;?>" onClick="this.form.action='<?=$CFG->wwwroot ."/transaction_status.php?confirmuser=".$sqlrow['customerid']."&orderid=".$sqlrow['orderid']."&datepurchase=".$sqlrow['date']."&resitexamid=".$finalexamid;?>'" onMouseOver="style.cursor='pointer'" type="submit" name="sahbeli" value="<?=get_string('confirm');?>"/>
						<?php }else{ ?>
						...
						if($_POST['sahbeli']){						
							...
							$resitexamid=$_GET['resitexamid'];
							
							// buy and extend a course
							...
							if($qselcourse['category']=='3'){
								...
								$resitsql="DELETE FROM {$CFG->prefix}quiz_attempts WHERE quiz='".$resitexamid."' AND userid='".$ucadidateid."'"; 
								$rsql=mysql_query($resitsql);
								if($rsql>0){
									$resitsql="DELETE FROM {$CFG->prefix}quiz_grades WHERE quiz='".$resitexamid."' AND userid='".$ucadidateid."'"; 
									$rsql=mysql_query($resitsql);
								}
								
								//resite///purchase detais to statement DB
								...
								if($ucount=='0'){
									...
									if($enrolupdate){
										...
										$to = $sqlrow['email'];
										$subject = "Notice: ".strtoupper($sqlrow['traineeid'])." - Successful Online Training Program Extension";
										$message = "
											<html>
											<head>
												<title>HTML email</title>
											...
											<br/><p>Candidate ID: ".strtoupper($sqlrow['traineeid'])."</p>
											
											<br/><p style='text-align:justify;'>We are pleased to inform you that your extension request for the subscription to the ".$sqldisplay['fullname']." is successful. We are pleased to extend the subscription for another 2 months. Please proceed to the training program as usual. </p>
												
											<br/><p style='text-align:justify;'>
											If you have opted to wire transfer the payment, this should be received by SHAPE<sup>&reg;</sup> no later than 30 days from the date you made the extension request. Once your payment has been received and cleared, you will be able to continue your online training program as usual.
											</p>

											<br/><p style='text-align:justify;'>
											If your account is not fully paid up within the stipulated deadline, SHAPE<sup>&reg;</sup> reserve the right to withdraw your extension request.  
											</p> 
