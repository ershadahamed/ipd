...
<?php
	$sinstitude=mysql_query("SELECT * FROM {$CFG->prefix}organization_type WHERE id='".$_GET['id']."' AND status='0'");
	$qins=mysql_fetch_array($sinstitude);
	
	$scountry=mysql_query("SELECT * FROM {$CFG->prefix}country_list");
	
	$groupofinstitution=$qins['groupofinstitution'];
	$iname=$qins['name'];
	$iaddress=$qins['address'];
	$iaddress2=$qins['address_line2'];
	$iaddress3=$qins['address_line3'];
	$icity=$qins['city'];
	$izip=$qins['zip'];
	$istate=$qins['state'];
	$icountry=$qins['country'];
	$itelephone=$qins['telephone'];
	$ifaxs=$qins['faxs'];
	$iorg_type=$qins['org_type'];
	$iwebsite=$qins['website'];
	$itimecreated=$qins['timecreated'];
	$imglink=$CFG->wwwroot.'/financialinstituition/'.$qins['path_logo'];
?>
<form id="myForm" onsubmit="return validateFormOnSubmit(this)" onChange="return displ();" action="editregister_admin.php" method="post" enctype="multipart/form-data">
<fieldset style="padding: 0.6em;" id="user" class="clearfix">
<legend style="font-weight:bolder;" class="ftoggler">Upload Logo</legend>
	<?php if($itimecreated!='0'){ ?>
...
<table width="100%" border="0">
  <tr>
	...
	<select name="groupofinstitution">
		...
		<?php
		$bil='1';
		while($qins2=mysql_fetch_array($sinstitude)){
		?>
		<option value="<?=$qins2['id'];?>" <?php if($groupofinstitution==$qins2['id']){ echo "selected"; } ?>><?=$qins2['name'];?></option>
		<?php
		}
		?>
	</select>
  ...
  <tr>
    ...
    <td width="1%">&nbsp;</td>
    <td>
		<input name="inst_name" type="text" id="inst_name" size="80%" value="<?=$iname;?>">
		<input name="inst_id" type="hidden" id="inst_id" size="80%" value="<?=$_GET['id'];?>">
	</td>
  </tr>
  <tr>
    <td valign="top"><?=get_string('address1');?>
      <?=$csign;?></td>
    <td>&nbsp;</td>
    <td><input name="inst_address" type="text" id="inst_address" size="80%" value="<?=$iaddress;?>" onKeyUp="javascript:this.value=this.value.toTitleCase()" /></td>
  </tr>
  <tr>
    <td valign="top"><?=get_string('address2');?>
      </td>
    <td>&nbsp;</td>
    <td><input name="inst_address2" type="text" id="inst_address2" size="80%" value="<?=$iaddress2;?>" onKeyUp="javascript:this.value=this.value.toTitleCase()" /></td>
  </tr>
  <tr>
    <td valign="top"><?=get_string('address3');?>
      </td>
    <td>&nbsp;</td>
    <td><input name="inst_address3" type="text" id="inst_address3" size="80%" value="<?=$iaddress3;?>" onKeyUp="javascript:this.value=this.value.toTitleCase()" /></td>
  </tr>  
  <tr>
    <td valign="top"><?=get_string('city');?>
      <?=$csign;?></td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_city" id="inst_city" value="<?=$icity;?>"></td>
  </tr>
   <tr>
    <td valign="top"><?=get_string('zip');?>
      <?=$csign;?></td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_zip" id="inst_zip" value="<?=$izip;?>"></td>
  </tr>
  <tr>
    <td valign="top"><?=get_string('state');?>
      <?=$csign;?></td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_state" id="inst_state" value="<?=$istate;?>"></td>
  </tr>
  <tr>
	...
	<select name="inst_country" id="inst_country">
		...
		?>
		<option value="<?=$qcountry['countrycode'];?>" <?php if($icountry==$qcountry['countrycode']){ echo 'selected';} ?>><?=$qcountry['countryname'];?></option>
		<?php
		}
		?>
	</select><?=$csign;?>
  ...
  <tr>
    <td valign="top">Telephone No.
      <?=$csign;?></td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_telephone" id="inst_telephone" value="<?=$itelephone;?>"></td>
  </tr>  
  <tr>
    <td valign="top">Faxs No</td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_faxs" id="inst_faxs" value="<?=$ifaxs;?>"></td>
  </tr>
  <tr>
    <td valign="top">Website</td>
    <td>&nbsp;</td>
    <td><input type="text" name="inst_website" id="inst_website" value="<?=$iwebsite;?>"></td>
  </tr>
  <tr>
    ...
    <td><select name="organization_type" id="organization_type">
      ...
      <option value="13" <?php if($iorg_type=='13'){ echo "Selected";} ?>>Financial Institution</option>
      <option value="12" <?php if($iorg_type=='12'){ echo "Selected";} ?>>Business Partner</option>
    </select></td>
  </tr>  
  <tr>
    <td>&nbsp;</td>
