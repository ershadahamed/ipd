...
/* $sqlsupportemail = mysql_query("SELECT * FROM {$CFG->prefix}config WHERE name='supportemail'");
  ...
  $afterresult = date('d-m-Y H:i:s', strtotime($resultout . " + 14 days"));  // 2 week after result reease
  ...
  $ucfullname = $sqluser['firstname'] . ' ' . $sqluser['middlename'] . ' ' . $sqluser['lastname'];
  } else {
...
if (isloggedin()) {
    ...
    if ($USER->id != '2') {
        foreach (get_coursedetails($USER->id) as $users) {
            ...
            if (!$DB->count_records('statement', array('courseid' => $courseid, 'candidateid' => $traineeid, 'remark' => 'Subscribe'))) {
                $record = new stdClass();
                $record->candidateid = $traineeid;
                $record->subscribedate = $registrationdate;
                $record->courseid = $courseid;
                $record->remark = $descsubcribe;
                $record->debit1 = $courseprice;
                $lastinsertid = $DB->insert_record('statement', $record, false);
            }

            if (!$DB->count_records('statement', array('courseid' => $courseid, 'candidateid' => $traineeid, 'remark' => 'Payment'))) {
                $record = new stdClass();
                $record->candidateid = $traineeid;
                $record->courseid = $courseid;
                $record->remark = $payment;
                $record->credit1 = $courseprice;
                $record->paymentdate = $enrollmenttime;
                $record->status = '1';
                $lastinsertid = $DB->insert_record('statement', $record, false);
            }
        }
