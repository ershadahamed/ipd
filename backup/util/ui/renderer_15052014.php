...
class core_backup_renderer extends plugin_renderer_base {
    ...
    public function render_backup_files_viewer(backup_files_viewer $viewer) {
		...
		//add by arizan;
		$sqlT=mysql_query("SELECT * FROM {$CFG->prefix}files WHERE component='backup' AND filename !='.' AND (filearea='course' OR filearea='activity') ORDER BY id DESC");
		$s=mysql_fetch_array($sqlT);
		//echo $s['filename'].'<br/>';
		//echo userdate($s['timemodified']).'<br/>';
		//echo display_size($s['filesize']).'<br/>';
		$fileurl2 = moodle_url::make_pluginfile_url($s['contextid'], $s['component'], $s['filearea'], null, $s['filepath'], $s['filename'], true);
		// echo $fileurl2;
        
        $table = new html_table();
        // $table->attributes['class'] = 'backup-files-table generaltable';
		$table->attributes['class'] = 'viewdata';
        $table->head = array(get_string('examinationtitle'), get_string('time'), get_string('size'), get_string('softwaredownload')/*, get_string('restore')*/);
        $table->width = '100%';
        $table->data = array();
		...
		//add by arizan
            $table->data[] = array(
                ...
                html_writer::link($fileurl2, '<center>'.get_string('download').'<center>')
                //remove by arizanabdullah on 16052012
		...
		//$sqluser=mysql_query("SELECT * FROM mdl_cifauser WHERE id='2'");
		//$sqlcount=mysql_fetch_array($sqluser);		
		//if($sqlcount['username'] == 'admin'){
