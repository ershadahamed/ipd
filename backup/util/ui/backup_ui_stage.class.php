    ...
    protected function initialise_stage_form() {
        ...
        if ($this->stageform === null) {
            ...
            foreach ($tasks as &$task) {
                ...
                if ($task instanceof backup_root_task) {
					echo '<div style="display:none;">'; // disable by arizan abdullah
...
class backup_ui_stage_complete extends backup_ui_stage_final {
    ...
    public function display() {
        ...
        default:
			...
			$restorerul = new moodle_url('/course/view.php', array(
            'id' => $this->get_ui()->get_controller()->get_courseid()));
        }
		...
		echo $OUTPUT->continue_button($restorerul, 'get');
        echo $OUTPUT->box_end();
    }
}
