...
class core_backup_renderer extends plugin_renderer_base {
    ...
    public function render_backup_files_viewer(backup_files_viewer $viewer) {
        ...
        $table->head = array('No', get_string('examinationtitle'), /* get_string('time'), get_string('size'), */ get_string('softwaredownload')/*, get_string('restore')*/);
        $table->width = '100%';
        $table->data = array();
        ...
        foreach ($files as $file) {
            ...
            $table->data[] = array(
                ...
                html_writer::link($fileurl, '<center>'.get_string('download').'</center>'),
