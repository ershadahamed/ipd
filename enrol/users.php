...
/*$qry = "SELECT * FROM mdl_cifauser_enrolments a, mdl_cifaenrol b, mdl_cifauser c, mdl_cifacourse d
		WHERE b.id=a.enrolid AND a.emailsent='0' AND a.userid=c.id AND b.courseid='".$COURSE->id."' AND d.id=b.courseid";
$sql = mysql_query($qry); 
...
while($rs = mysql_fetch_array($sql)){ 
	...
	//include('email_notification.php');
	$from=$rowAdmin['email'];  //administrator mail
	...
	$message = "You have been enroll for this courses:- <br/><b>$coursename($shortcoursename)</b>. <br/><br/>
				Now, you <b>$namesent</b> can login CIFA ONLINE for more details. <a href='".$CFG->wwwroot."' target='_blank'>Login to cifa</a><br/>
			<p>Thank you.</p>
			";
