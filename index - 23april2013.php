...
if	($frontpagelayout){
	if(($USER->id)!='2'){
		...
		if(($qrole->roleid !='5')){ 
			...
			//echo $count;
			if($count == '0'){
				//$firstaccess = strtotime('now');
				//$sqlp=mysql_query("UPDATE {$CFG->prefix}user SET firstaccess='".time()."' WHERE firstaccess='0' AND id='".$USER->id."'");
				/*$returnto = "$CFG->wwwroot/login/change_password.php?id=1";
				redirect($returnto);*/
