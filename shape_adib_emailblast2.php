...
while($rs1 = mysql_fetch_array($sql1)){
	...
	$qry2 = "SELECT b.id as userid, b.firstname, b.lastname, b.traineeid, 
	...
	$qry3 = "SELECT id FROM mdl_cifaquiz_attempts
			WHERE userid='".$rs2['userid']."' AND quiz='26' LIMIT 1";
	...
	echo "<br>".$rs2['firstname']." ".$rs2['lastname']."=".$rs2['email']."=".$rs2['traineeid']."=".$rs2['startdate'].">".$newenddate;
			...
			$subject = "Reminder: ".strtoupper($rs2['traineeid'])." - Expiry of SHAPE Foundation of Islamic Banking and Finance Course Subscription";
						...
						$message = "
							...
							<p style='text-align:justify;'>This is a brief reminder that you have not completed the SHAPE<sup>&reg;</sup> Foundation of Islamic Banking & Finance e-learning course and the access is due to expire by ".$newenddate.". Once expired, you will not be able to access the course online or take the test.</p>
							
							<p style='text-align:justify;'>The final test and test strengthen your knowledge in Islamic Banking. The test is part of the online course, hence no additional payment is required. Each candidate will be entitled to 2 test opportunities and mock test to familiarize yourself with the online testing environment. To launch the test, simply click the <u>Final Test</u> link available under <strong>My Training</strong> at the home page. </p>
							
							<p style='text-align:justify;'>Please note the test validity is same with the course subscription period. After which, the unused test token will expire and you will not have access to the online course or the test. </p> 
							
							<p style='text-align:justify;'>To start the course and attempt the test, login to the SHAPE<sup>&reg;</sup> (IPD) portal with your provided username and password via the following link: <strong><u><a href='http://134.213.66.124/shapeipd/login/index.php' target='_blank'>http://134.213.66.124/shapeipd/login/index.php</a></u></strong></p> 
