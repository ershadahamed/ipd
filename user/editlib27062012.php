...
function useredit_shared_definition(&$mform, $editoroptions = null) {
	...
	if($USER->id != '2'){
		...
		$mform->addElement('text', 'usercategoryid', $usercategorytitle, 'maxlength="70" size="25" readonly="readonly"');
		...
		$mform->addElement('select', 'usercategoryid', $usercategorytitle, $usercategory);
		$mform->addRule('usercategoryid', $strrequired, 'required', null, 'client');
		if (!empty($CFG->usercategoryid)) {
			$mform->setDefault('usercategoryid', $CFG->usercategoryid);
		}
	...
	$usertypelist=array();
	$usertypelist['0']='Select a user type..';
	...
	if($USER->id != '2'){
		//$mform->addElement('select', 'usertype', $usertypetitle, $usertypelist, 'disabled');
	...
	//add by arizan abdullah 24/10/2011 ************************************************************************************************
	/*$sqlD=mysql_query("Select id, usertype From mdl_cifauser");
	$resultD=mysql_fetch_array($sqlD);
	if($resultD['id']== $userid){
	
	$usertype = array();
	$usertype['administrator'] = get_string('usertypeadministrator');
    $usertype['active trainee'] = get_string('usertypeactivetrainee');
	$usertype['inactive trainee'] = get_string('usertypeinactivetrainee');
	$usertype['prospect trainee'] = get_string('usertypeprospecttrainee');
	$usertype['offline trainee'] = get_string('usertypeofflinetrainee');
	$usertype['graduate trainee'] = get_string('usertypegraduatetrainee');
    $usertype['trainer'] = get_string('usertypetrainer');
	$usertype['examiner'] = get_string('usertypeexaminer');
    $usertype['reviewer'] = get_string('usertypereviewer');
	$usertype['marker'] = get_string('usertypemarker');
	$usertype['exam centre admin'] = get_string('examcentreadmin');
	$usertype['help desk admin'] = get_string('helpdeskadmin');
	$usertype['institutional clients'] = get_string('institutionalclients');
	$usertype['business partners'] = get_string('businesspartners');

    $mform->addElement('select', 'usertype', get_string('usertype'), $usertype);
    $mform->addHelpButton('usertype', 'usertype');
	$mform->setDefault('usertype', 'active trainee');
    ...
    $mform->addElement('select', 'usertype', get_string('usertype'), $usertype, 'disabled');
    $mform->addHelpButton('usertype', 'usertype');
	$mform->setDefault('usertype', 'active trainee');

	$mform->addElement('text', 'traineeid', get_string('traineeid'), 'maxlength="70" size="25" disabled="disabled"');
    $mform->setType('traineeid', PARAM_NOTAGS);
	//$mform->addRule('traineeid', $strrequired, 'required', null, 'client');	
	}*/
	//***************************************************************************************************************************
