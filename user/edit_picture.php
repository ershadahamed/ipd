...
require_once($CFG->dirroot.'/user/editlib_picture.php');
...
if ($course->id != SITEID) {
    ...
    // $PAGE->set_pagelayout('standard'); //added by aa
	
	$url = new moodle_url('/user/edit.php', array('id'=>$USER->id, 'course'=>$course->id)); //added by arizan
	$urleditpicture = new moodle_url('/user/edit_picture.php', array('id'=>$USER->id, 'course'=>$course->id));
	
	$PAGE->set_pagelayout('edit_picture');
	$PAGE->navbar->add(get_string('myprofile'))->add(get_string('profile'), $url)->add(get_string('mypicture'), $urleditpicture);
}
