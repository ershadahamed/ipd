...
if(!$currentuser) {
	...
	if($user->gender == '0'){ print_row(get_string('gender') . ':', 'Male');
	}else{ print_row(get_string('gender') . ':', 'Female'); }
	...
	//if (has_capability('moodle/user:viewhiddendetails', $context)) {
		...
		// }else{ print_row(get_string('phone') . ':', "-"); }
	...
	}else{ print_row(get_string('state') . ':', "-"); }
	...
	if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
		...
		}else{ print_row(get_string('designation') . ':', "-"); }
		
		if (($user->department)!='') {		
				print_row(get_string('department') . ':', $user->department);
		}else{ print_row(get_string('department') . ':',' - '); } //arizan
		...
		if ($user->phone1!='') {
			print_row(get_string("mobiletel").":", "$user->mobile");
		}else{ print_row(get_string("mobiletel").":", "-"); }	
	}
}
echo '</table></fieldset>'; 
...
//if(!$currentuser) {
	if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
		...
		if (($user->empstatus)!='') {
			if(($user->empstatus)=='1'){ print_row(get_string('empstatus') . ':', get_string('working')); }
			if(($user->empstatus)=='2'){ print_row(get_string('empstatus') . ':', get_string('notworking')); }
			if(($user->empstatus)=='3'){ print_row(get_string('empstatus') . ':', get_string('student')); }	
		}else{ print_row(get_string('empstatus') . ':', '-'); }
		
		if (($user->empname)!='') {
			print_row(get_string('empname') . ':', $user->empname);
		}else{ print_row(get_string('empname') . ':', '-'); }
		...
		if (($user->department)!='') {
			...
			}else{ print_row(get_string('department') . ':', $user->department); } //arizan
		}else{ print_row(get_string('department') . ':', '-'); }
		
		if (($user->empstartdate)!='') {
			if(($user->empstatus)!='1'){
				print_row(get_string('empstartdate') . ':',' - ');
			}else{ print_row(get_string('empstartdate') . ':', date('d-m-Y', $ustartdate)); }
		}else{ print_row(get_string('empstartdate') . ':', '-'); }
		echo '</table></fieldset>'; 
	}
//}
...
if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center
	...
	if (($user->highesteducation)!='0') {
		if(($user->highesteducation)=='1'){ print_row(get_string('highesteducation') . ':', get_string('highesteducation1')); }
		if(($user->highesteducation)=='2'){ print_row(get_string('highesteducation') . ':', get_string('highesteducation2')); }
		if(($user->highesteducation)=='3'){ print_row(get_string('highesteducation') . ':', get_string('highesteducation3')); }
		if(($user->highesteducation)=='4'){ print_row(get_string('highesteducation') . ':', get_string('highesteducation4')); }
		if(($user->highesteducation)=='5'){ print_row(get_string('highesteducation') . ':', get_string('highesteducation5')); }
	}else{ print_row(get_string('highesteducation') . ':', '-'); }
	...
	}else{ print_row(get_string('yearcomplete') . ':', '-'); }
	
	if (($user->professionalcert)!='') {
		if(($user->professionalcert)=='1'){ print_row(get_string('professionalcert') . ':', get_string('certificate0')); }
		if(($user->professionalcert)=='2'){ print_row(get_string('professionalcert') . ':', get_string('certificate1')); }
		if(($user->professionalcert)=='3'){ print_row(get_string('professionalcert') . ':', get_string('certificate2')); }
		if(($user->professionalcert)=='4'){ print_row(get_string('professionalcert') . ':', get_string('certificate3')); }
		if(($user->professionalcert)=='5'){ print_row(get_string('professionalcert') . ':', get_string('certificate4')); }
		if(($user->professionalcert)=='6'){ print_row(get_string('professionalcert') . ':', get_string('certificate5')); }
		if(($user->professionalcert)=='7'){ print_row(get_string('professionalcert') . ':', get_string('certificate6')); }
		if(($user->professionalcert)=='8'){ print_row(get_string('professionalcert') . ':', get_string('certificate7')); }
		if(($user->professionalcert)=='9'){ print_row(get_string('professionalcert') . ':', get_string('certificate8')); }
		if(($user->professionalcert)=='10'){ print_row(get_string('professionalcert') . ':', get_string('certificate9')); }
		if(($user->professionalcert)=='11'){ print_row(get_string('professionalcert') . ':', get_string('certificate10')); }
		if(($user->professionalcert)=='12'){ print_row(get_string('professionalcert') . ':', get_string('certificate11')); }	
	}else{ print_row(get_string('professionalcert') . ':', '-'); }
	...
	}else{ print_row(get_string('nameofqualification') . ':', '-'); }
	...
	}else{ print_row(get_string('college') . ':', '-'); }

	if (($user->major_edu)!='') {
		print_row(get_string('major') . ':', $user->major_edu);
	}else{ print_row(get_string('major') . ':', '-'); }
	...
	}else{ print_row(get_string('completiondate') . ':', '-'); }
	echo '</table></fieldset>'; 
	//}	
}else{ //if exam center	
	if($currentuser){
		...
		}else{ print_row(get_string("institutionname").":", "-"); }	

		if ($user->address!='') {
			print_row(get_string("address").":", "$user->address");
		}else{ print_row(get_string("address").":", "-"); }	

		if ($user->address2!='') {
			print_row(get_string("address").":", "$user->address2");
		}	
		if ($user->address3!='') {
			print_row(get_string("address").":", "$user->address3");
		}	
		
		if ($user->orgtel!='') {
			print_row(get_string("orgtel").":", "$user->orgtel");
		}else{ print_row(get_string("orgtel").":", "-"); }	
		
		if ($user->orgfax!='') {
			print_row(get_string("orgfax").":", "$user->orgfax");
		}else{ print_row(get_string("orgfax").":", "-"); }	
		...
		}else{ print_row(get_string("website").":", "-"); }	
		echo '</table></fieldset>'; 
	}
}
...
if($currentuser) {
	if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center
		...
		if (($user->compreference3)!='') {
				print_row('<input type="checkbox" name="cm1" value="1" checked disabled /> ', '<div style="text-align:justify;">'.get_string('compreference3').'</div>');
		}
		echo '</table></fieldset>';
	}
}
...
<input type="submit" name="editprofile" onClick="this.form.action='<?=$CFG->wwwroot. '/user/edit.php?id='.$user->id.'&course=1';?>'" value="<?=ucwords(strtolower(get_string('editmyprofile')));?>" />
