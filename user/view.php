...
$getpage = optional_param('getpage', 0, PARAM_INT);   // user id
...
if ($courseid == SITEID) {   // Since Moodle 2.0 all site-level profiles are shown by profile.php
    if ($id == $USER->id) {
        ...
        // redirect($CFG->wwwroot.'/user/profile.php?id='.$id);  // Immediate redirect
        if($USER->id == '2'){
        redirect($CFG->wwwroot . '/userfrontpage/updatecandidate_details.php?getpage='.$getpage);
        }else{
        redirect(new moodle_url('/candidatemanagement/cifacandidatemanagement.php?id=' . $id.'&'.$getpage)); // Immediate redirect
        }
    }
    // redirect($CFG->wwwroot . '/user/profile.php?id=' . $id);  // Immediate redirect
}

$PAGE->set_url('/user/view.php', array('id' => $id, 'course' => $courseid, 'getpage'=>$getpage));
