...
require_once($CFG->dirroot.'/user/edit_form_roles.php');
...
if ($userid == $USER->id){ 									//modi by arizan
	require_once($CFG->dirroot.'/user/editlib.php');
}else{
	require_once($CFG->dirroot.'/user/editlib_roles.php');
}
...
if ($course->id != SITEID) {
	...
	//breadcrum
	$url = new moodle_url('/user/edit.php', array('id'=>$USER->id, 'course'=>$course->id)); //added by arizan
	$urlprofile=new moodle_url('/user/profile.php', array('id'=>$USER->id)); 
	if ($user->id == $USER->id) {
		if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
			$PAGE->navbar->add(get_string('mycandidate'), $urlprofile)->add(get_string('profile'), $url); 
		}else{
			$PAGE->navbar->add(get_string('myaccount'), $urlprofile)->add(get_string('profile'), $url); 		
		}	
	}else{
		if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
			$PAGE->navbar->add(get_string('mycandidate'), $urlprofile)->add(get_string('profile'));
		}else{
			$PAGE->navbar->add(get_string('myaccount'), $urlprofile)->add(get_string('updatedetails'), $url); 		
		}		
	}
}
...
if ($user->id == $USER->id) {
    ...
    if (!has_capability('moodle/user:editownprofile', $systemcontext)) {
      //mmn turn off this  -->print_error('cannotedityourprofile');
    }
...
if ($user->id == $USER->id) { //modi by arizan
	$userform = new user_edit_form(null, array('editoroptions'=>$editoroptions));
}else{
	$userform = new user_edit_form_roles(null, array('editoroptions'=>$editoroptions));
}
...
// added by arizan for cancel button
if ($userform->is_cancelled()){
	if ($user->id == $USER->id) {
		if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
			redirect(new moodle_url('/index.php'));
		}else{
			redirect(new moodle_url('/candidatemanagement/cifacandidatemanagement.php?id='.$USER->id));		
		}
	}else{
		if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){	//jika bkn exam center//business partner//hr admin
			redirect(new moodle_url('/index.php'));
...
if ($usernew = $userform->get_data()) {
	...
	//for Education background & Employment details
	$sqlupdateusr=mysql_query("
		UPDATE mdl_cifauser 
		SET 
...
/// Finally display THE form
	echo '<div style="color: #AA0000; text-align: right;">'.get_string('somefieldsrequired_form', 'form').'</div>'; //added by arizan
    //echo $_GET['id'].'-'.$user->id.'-'.$USER->id;
