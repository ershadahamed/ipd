...
function useredit_shared_definition(&$mform, $editoroptions = null) {
	...
	if($USER->id=='2'){
		...
		if($rs['roleid']!='10'){ //jika bukan exam center
...
/* 		$mform->addElement('checkbox', 'deletepicture', get_string('delete'));
	...
	if($rs['roleid']!='10'){ //jika bukan exam center
		...
		$mform->setDefault('title', $CFG->defaulttitle);	
	}
    ...
    if (!empty($CFG->emailchangeconfirmation) and !empty($user->preference_newemail)) {
		...
		if($rs['roleid']!='10'){	//jika bkn exam center
	...
	if($rs['roleid']!='10'){	
		...
		$mform->addElement('text', 'empname', get_string('empname'), 'maxlength="50" size="40"');
		$mform->setType('empname', PARAM_NOTAGS);
		$mform->disabledIf('empname', 'empstatus', 'noteq', 1);

		$mform->addElement('text', 'designation', get_string('designation'), 'maxlength="50" size="40"');
		$mform->setType('designation', PARAM_NOTAGS);	
		$mform->disabledIf('designation', 'empstatus', 'noteq', 1);
		
		$mform->addElement('text', 'department', get_string('department'), 'maxlength="50" size="40"');
		$mform->setType('department', PARAM_NOTAGS);	
		$mform->disabledIf('department', 'empstatus', 'noteq', 1);	
		
		$mform->addElement('date_selector', 'empstartdate', get_string('empstartdate')); //start date
		...
		$mform->addElement('text', 'phone1', get_string('worktel'), 'maxlength="20" size="25"');
		$mform->addRule('phone1', $strrequired, 'required', null, 'client');
		$mform->setType('phone1', PARAM_NOTAGS); //work phone

		$mform->addElement('text', 'phone2', get_string('workfax'), 'maxlength="20" size="25"');
		$mform->addRule('phone2', $strrequired, 'required', null, 'client');
		$mform->setType('phone2', PARAM_NOTAGS); //work fax

		$mform->addElement('text', 'mobile', get_string('mobiletel'), 'maxlength="20" size="25"');
		$mform->addRule('mobile', $strrequired, 'required', null, 'client');
		$mform->setType('mobile', PARAM_NOTAGS); //mobile phone
		...
		$mform->addElement('text', 'orgtel', get_string('orgtel'), 'maxlength="20" size="25"');
		$mform->setType('orgtel', PARAM_NOTAGS); //orgtel

		$mform->addElement('text', 'orgfax', get_string('orgfax'), 'maxlength="20" size="25"');
	...
	if($rs['roleid']!='10'){
		...
		$listyearsedu = array_combine(range($currentyeardateedu, 1933), range($currentyeardateedu, 1933));
		//array_unshift($listyearsedu, "Select one..");
		$mform->addElement('select', 'yearcomplete_edu', get_string('yearcomplete'), $listyearsedu);	
		if (!empty($USER->yearcomplete_edu)) {
			$mform->setDefault('yearcomplete_edu', $USER->yearcomplete_edu);
		}	
		...
		//array_unshift($listyears, "Select one..");
		...
		$mform->addElement('date_selector', 'startdate_edu', get_string('empstartdate')); //start date	
		
		$mform->addElement('date_selector', 'completion_edu', get_string('completiondate')); //start date		
	}
	...
	if($rs['roleid']!='10'){
		...
		$mform->addElement('advcheckbox', 'cpreference1', null,  get_string('compreference1'), array('group' => 1, 'disabled'=>'disabled'));
		...
		$mform->addElement('advcheckbox', 'cpreference3', null,  get_string('compreference3'), array('group' => 1, 'disabled'=>'disabled'));
