...
function useredit_shared_definition(&$mform, $editoroptions = null) {
	...
	$choices['2'] = 'Miss';
	if($USER->id!='2'){
		$mform->addElement('select', 'title', 'Title', $choices, 'disabled="disabled"');
	}else{
    ...
    if (fullname($nameordercheck) == 'b a' ) {  // See MDL-4325
		...
		if($USER->id != '2'){
				$mform->addElement('static','firstname', get_string('firstname'), 'maxlength="26" size="40"');
				if(($CFG->middlename)!=''){ $mform->addElement('static','middlename',  get_string('middlename'),  'maxlength="26" size="40"');}
				else{ $mform->addElement('text','middlename',  get_string('middlename'),  'maxlength="26" size="40"'); }				
				$mform->addElement('static','lastname',  get_string('lastname'),  'maxlength="26" size="40"');
		}else{
	...
	if(($rs['roleid']!='10') && ($rs['roleid']!='12') && ($rs['roleid']!='13')){
		...
		$mform->addElement('text', 'empname', get_string('empname'), 'placeholder="if working, fillup this box" maxlength="50" size="40"');
		$mform->setType('empname', PARAM_NOTAGS);
		$mform->disabledIf('empname', 'empstatus', 'noteq', 1);
			
		$mform->addElement('text', 'designation', get_string('designation'), 'placeholder="if working, fillup this box" maxlength="50" size="40"');
		$mform->setType('designation', PARAM_NOTAGS);	
		$mform->disabledIf('designation', 'empstatus', 'noteq', 1);
		
		$mform->addElement('text', 'department', get_string('department'), 'placeholder="if working, fillup this box" maxlength="50" size="40"');
		...
		$mform->addElement('date_selector', 'empstartdate', get_string('empstartdate'), $date_selector_modify, 'title="if working, fillup this box"'); //start date
		...
		$worktelcode=array();
		$worktelcode[] =& $mform->createElement('static', 'worktelcode', '', '+'.$ccode['iso_countrycode']);
		$worktelcode[] =& $mform->createElement('text', 'orgtel', '');
		$mform->addGroup($worktelcode, 'worktelcode', get_string('worktel'), ' ', false);	 //orgtel	
		
		$orgfaxcode=array();
		$orgfaxcode[] =& $mform->createElement('static', 'orgfaxcode', '', '+'.$ccode['iso_countrycode']);
