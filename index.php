...
if ($frontpagelayout) {
    if (($USER->id) != '2') {
        ...
        if (($qrole->roleid != '5')) {
            ...
            $qs = "SELECT * FROM mdl_cifauser WHERE firstaccess='0' AND id='" . $USER->id . "'";
            $sql = mysql_query($qs);
...
//Subscribe///purchase detais to statement DB
if (isloggedin()) {
    // $usertoreset='184';
    $duration = '120';
    // resetUserRegistration($usertoreset, $duration);
    /*if($USER->id == '2'){
        //$userid='206';
        //print_r(getThisUserInfo($userid));
        
        //updateThisUserPassword($userid, $duration, '309e07d3eb890acb9d0b679d8a3f253e');
        $courseid = '63';
        $scorms = viewScormDetails($courseid);
        foreach($scorms as $scorm){
            updateScormColumn($scorm->id, $courseid);
            print_r($scorm->id); echo "-";
            print_r($scorm->course);echo "-";
            print_r($scorm->displayattemptstatus); echo "-";
            print_r($scorm->skipview);echo "-";
            print_r($scorm->hidetoc);echo "-";
            print_r($scorm->auto);echo "-";
            print_r($scorm->reference);
            echo "<br/>";
        }
    }*/
    if ($USER->id != '2') {
        foreach (get_coursedetails($USER->id) as $users) {
            $traineeid = strtoupper($users->traineeid);
            $courseid = $users->courseid; 
            $courseprice = $users->cost;
            $currency = $users->currency;
            $registrationdate = $users->registrationdate;
            $enrollmenttime = $users->enrollmenttime;
            $descsubcribe = 'Subscribe';
            $payment = 'Payment';
            
            $countsubscribe = $DB->count_records('statement', array('courseid' => $courseid, 'candidateid' => $traineeid, 'remark' => 'Subscribe'));
            if ($countsubscribe == 0) {
                $desc = $descsubcribe.' - '.$users->fullname;
                $sql="INSERT INTO {$CFG->prefix}statement (transactionid, candidateid, subscribedate, courseid, remark, debit1, description) "
                . "VALUES ('".generateTransactionId()."', '".$traineeid."', '".$registrationdate."', '".$courseid."', '".$descsubcribe."', '".$courseprice."', '".$desc."')";
                mysql_query($sql);
            }

            if ($DB->count_records('statement', array('courseid' => $courseid, 'candidateid' => $traineeid, 'remark' => 'Payment')) == 0) {
                
                $desc = $payment.' - '.$users->fullname;
                $sql="INSERT INTO {$CFG->prefix}statement (transactionid, candidateid, courseid, remark, credit1, paymentdate, status, description) "
                . "VALUES ('".generateTransactionId()."', '".$traineeid."', '".$courseid."', '".$payment."', '".$courseprice."', '".$enrollmenttime."', '1', '".$desc."')";
                mysql_query($sql);
            }
        }
    }
