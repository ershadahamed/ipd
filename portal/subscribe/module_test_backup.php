<?php include('pageconfig/moduleHeader.php'); ?>	
	...
	?>
		...
		<tr>
			...
			<td>
				<?php
					$categoryModule = $rowModuleTest['category'];
					$courseModuleTest = $rowModuleTest['courseid'];
					$sqlTest=mysql_query("
					Select 
						a.category,
						b.course,
					...
					From 
						...
						mdl_cifaquiz b
					Where 
						a.category='$categoryModule' And
						b.course='$courseModuleTest'
					");
					while($rowTest=mysql_fetch_array($sqlTest)){
						echo $rowTest['name'];?>
						<input type="hidden" name="modulename" value="<?php echo $rowTest['name']; ?>">	
						<?php
					}
				?>
		...
		<tr>
			<td>Attempts Allow</td><td>:</td>
			<td>
				<?php echo $rowTest['attempts']; ?>
				<input type="hidden" name="attempts" value="<?php echo $rowModuleTest['attempts']; ?>">
				
				<?php
					$sqlTest2=mysql_query("
					Select 
						a.category,
						b.course,
						b.name,
					...
					Where 
						...
						b.course='$courseModuleTest'
					");
					while($rowTest2=mysql_fetch_array($sqlTest2)){
						echo $rowTest2['attempts'];?>
						<input type="hidden" name="attempts" value="<?php echo $rowTest2['attempts']; ?>">	
						<?php
					}
				?>				
