...
<!--*************************************************FORM AREA*********************************************************************************-->
<form action="subscribedetails.php" method="post">
...
<fieldset id="fieldset"><legend id="legend">Candidate Information</legend>
	<table id="formpayment">
	<tr><td width="20%">First Name			</td><td width="1%">:</td><td><?php echo $trainee_name; ?><input type="hidden" name="firstname" value="<?php echo $trainee_name; ?>" size="40" /></td></tr>
	...
	<tr><td>Phone no.			</td><td width="1%">:</td><td><?php echo $phone; ?><input type="hidden" name="phone_1" value="<?php echo $phone; ?>" /></td></tr>
...
<p align="center">
<input type="button" name="submit" value="Confirm & Proceed" title="Click 'Confirm & Proceed' to make a payment through paypal" target="_blank" onclick="window.open('<?php echo $CFG->wwwroot. "/userpolicy_purchase.php?"; ?>pay_method=<?=$pay_method;?>&&traineeID=<?=$traineeID;?>&&trainee_name=<?=$trainee_name;?>&&lastname=<?=$lastname;?>&&dob=<?=$dob;?>&&address=<?=$address;?>&&address2=<?=$address2;?>&&email=<?=$email;?>&&phone=<?=$phone;?>&&city=<?=$city;?>&&province=<?=$province;?>&&postal=<?=$postal;?>&&country=<?=$country;?>&&cost=<?=$cost;?>&&currency=<?=$currency;?>&&PaypalBusinessEmail=<?=$PaypalBusinessEmail;?>&&coursename=<?php echo $row['fullname'];?>&&shortname=<?php echo $row['shortname']; ?>&&courseid=<?php echo $row['id']; ?>', 'Window2', 'width=820,height=400,resizable = 1, scrollbars=yes');" />
</p>
</form>
<!---user policy popup---->
    <script type="text/javascript" src="http://gettopup.com/releases/latest/top_up-min.js"></script>
    <div id="examples">
		<?php $link2= $CFG->wwwroot. "/userpolicy_purchase.php?traineeID=".$traineeID."&&trainee_name=".$trainee_name."&&lastname=".$lastname."&&address=".$address."&&dob=".$dob."&&address2=".$address2."&&email=".$email."&&phone=".$phone."&&city=".$city."&&province=".$province."&&postal=".$postal."&&country=".$country."&&cost=".$cost."&&currency=".$currency."&&PaypalBusinessEmail=".$PaypalBusinessEmail."&&coursename=".$row['fullname']."&&shortname=".$row['shortname']."&&courseid=".$row['id']; ?>
		<a href="<?=$link2;?>" class="tu_iframe_1000x600">User policy</a>
    </div>
    <script type="text/javascript">
      TopUp.addPresets({
        "#examples a": {
          title: "SHAPE<sup>TM</sup> Online Training Program User Policies – Please Read Before Purchasing",
          group: "examples",
          readAltText: 1,
          shaded: 1
        }
