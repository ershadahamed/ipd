<body onLoad="window.print()">
    ...
    //global $PAGE;
    //echo $PAGE->requires->css('/organization/stylesheet.css');
...
// echo $OUTPUT->header();
    ...
    $todaytime = time();
    //$records = buildsupport_activities($userid);
    $author = createdbyuser($logginuserid);
    
    if($formtype == get_string('user')){
        $records = buildsupport_activities($userid);
    }elseif($formtype == get_string('organization')){
        $records = buildsupport_activities_org($userid);
    }      
    
    echo '<h2>' . get_string('activities') . '</h2>';
...
// content
    foreach($records as $record){
        ...
        $table->data[] = array(
            $timecreated, getActivitiyId($record->id), $activitiescategory, $activitiesdesc, $author
        );    
    }
