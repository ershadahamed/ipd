   ...
   include('../manualdbconfig.php');
    ...
    if($COURSE->id == '12'){ //exam and question bank section on course
        if($rs['roleid']=='10' || ($USER->id == '2')){
           ...
           // $sqlOperation="SELECT *, date_format(from_unixtime(timecreated), '%Y-%m-%d') as timecreated FROM mdl_cifabackup_controllers";
           // $sqlOperation.=" WHERE operation = 'backup' AND type='course' AND itemid='".$COURSE->id."'";
            //$sqlOperation.=" ORDER BY id DESC";
            $sqlRw=mysql_query($sqlOperation);
            $rs1=mysql_fetch_array($sqlRw);
            
            $oneMonthOn = date('Y-m-d',strtotime($rs1['timemodified'] . " + 2 day"));
			if(($rs1['timecreated']<=date('Y-m-d'))&& (date('Y-m-d')<=$oneMonthOn)){
            
            echo 'Date created token: '.$rs1['timemodified'].' <br/>Date today: '.date('Y-m-d');   
            echo '<br/> Date expired: '.$oneMonthOn; 
                
            echo '<br/><br/>';
            //$linktoken=$CFG->wwwroot.'/pluginfile.php/19/backup/course/download-moodle2-course-cifa_foundation_curriculum_exam-20120515-1902.mbz?forcedownload=1';
            //echo '<ul><a href="'.$linktoken.'" target="_blank" title="Download backup question">>>Link to download question - available for examcentre admin & administrator</a></ul>';
            
            //display - list of backup/download
            //include($CFG->wwwroot . '/backup/restorefile_form.php');
           // include($CFG->wwwroot . '/backup/util/includes/restore_includes.php');      
            ...
            echo '<br/>';
			}else{ echo '- No files to download. ';}
