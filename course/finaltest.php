<?php
	...
	$viewresult=get_string('mockexam');
	$title="$SITE->shortname: ".$viewresult;
	$PAGE->set_title($title);
	$PAGE->set_heading($site->fullname);
	$PAGE->set_pagelayout('buy_a_cifa');
	$urlmockexam = new moodle_url('/course/mock_exam.php', array('id'=>$USER->id)); //added by arizan	
	...
	$PAGE->navbar->add(get_string('mytraining'), $urlmytraining)->add(get_string('mockexam'), $urlmockexam);
	...
	a:hover {text-decoration:underline;}
		...
		<?php
			...
			$selectnew=mysql_query("			
					...
					Where
					  a.category = '3' And a.visible = '1' And e.userid='".$USER->id."'
			  ");				
        ...
        <table id="availablecourse" style="margin-bottom:1em;">
			<tr class="yellow">
                ...
                <th class="adjacent" width="20%" style="text-align:center"><?=get_string('view');?></th>
            </tr>
			<?php
			...
			{				
				...
				");	
				// echo $serow['courseid'].$serow['quizid'];
                ...
                <tr>
					...
					<?php					
						$sqlattempts=mysql_query("
							...
							Where
							  a.id='".$serow['cifaquizid']."' AND a.course = '".$serow['course1']."' AND b.userid = '".$serow['userid']."'							  
						");
						$cattempts=mysql_num_rows($sqlattempts);
						// echo $serow['attempts'].'=='.$cattempts;
						if($serow['attempts']==$cattempts){  // if attempts is 2
							echo 'Limited to attempt';		
							$access_token=uniqid(rand());
							$tokencreated=strtotime('now'); //token start
							$tokenexpiry=strtotime(date('d-m-Y H:i:s',$tokencreated) . " + 2 months"); //token expiry
							
							$sqlUP=mysql_query("UPDATE {$CFG->prefix}user_accesstoken SET timecreated_token='".$tokencreated."', tokenexpiry='".$tokenexpiry."' WHERE userid='".$serow['userid']."' AND courseid='".$serow['courseid']."'") 
