...
function print_log_xls($course, $user, $date, $order = 'l.time DESC', $modname, $modid, $modaction, $groupid) {
    ...
    foreach ($logs['logs'] as $log) {
        ...
        $myxls->write($row, 5, make_log_url($log->module, $log->url), ''); // make by arizan here!!!
...
function print_overview($courses, array $remote_courses = array()) {
    ...
    foreach ($remote_courses as $course) {
        ...
        echo $OUTPUT->heading(html_writer::link(
                        new moodle_url('/auth/mnet/jump.php', array('hostid' => $course->hostid, 'wantsurl' => '/course/view.php?id=' . $course->remoteid)), format_string($course->shortname), $attributes) . ' (' . format_string($course->hostname) . ')', 3);
        echo $OUTPUT->box_end();
    }
}
...
function get_all_sections($courseid) {
    ...
    if (!array_key_exists($courseid, $coursesections)) {
        $coursesections[$courseid] = $DB->get_records("course_sections", array("course" => "$courseid"), "section", "section, id, course, name, summary, summaryformat, sequence, visible");
    }
 ...
 */
function print_section_completion($course, $section, $mods, $modnamesused, $absolute = false, $width = "100%", $hidecompletion = false) {
        ...
        foreach ($sectionmods as $modnumber) {
            ...
            if ($mod->uservisible) {
                ...
                if ($url = $mod->get_url()) {
                    ...
                    if ($modulename == 'SCORM package') {
                        //added by arizan

                        $sco_launch = mysql_query("SELECT * FROM mdl_cifascorm WHERE scormtype ='local' AND course = '" . $course->id . "' AND name = '" . $instancename . "'");
                        $sco_a = mysql_fetch_array($sco_launch);
                        $ab = $sco_a['launch'] + '1';
                        $url3 = $CFG->wwwroot . '/mod/scorm/player.php?scoid=' . $ab . '&cm=' . $mod->id;


                        $scorm_percent = mysql_query("
						Select
					  b.*,
					  a.name
					From
					...
					Where
					  ...
					  a.scormtype = 'local'	And  b.scoid='" . $ab . "' And b.value='completed'			
					");
                        $s_percent = mysql_num_rows($scorm_percent);

                        $exam_percent = mysql_query("
					Select
					  b.*,
					  b.timefinish As timefinish1
					From
					...
					Where
					  ...
					  b.timefinish != '0'
					");
                        $e_percent = mysql_num_rows($exam_percent);

                        if ($USER->id === '2') {
                            echo '<a' . $linkcss . $mod->extra . $onclick .
                            ' href="' . $url3 . '"><img src="' . $mod->get_icon_url() .
                            '" class="activityicon" style="display:none; alt="' .
                            $modulename . '" /> ' .
                            $accesstext . '<span class="instancename" style="display:none;">' .
                            $instancename . $altname . '</span></a>';
                        } else {
                            echo '<input type="hidden" name="' . $instancename . '" value="' . $mod->get_icon_url() . '" />';
                            echo '<img src="' . $mod->get_icon_url() .
                            '" class="activityicon" style="display:none;" alt="' .
                            $modulename . '" /> ' .
                            $accesstext . '<span class="instancename" style="display:none;">' .
                            ...
                            // print_r(getScormId($course->id, $instancename)->id); // arizan
                            //$scoid=(getScormId($course->id, $instancename)->launch) + 1;
                            $scormid = getScormId($course->id, $instancename)->id;

                            print_r(attemptscormpercent($USER->id, $scormid));   // display percentage// arizan 
                        }
                    } else {
                        echo '<input type="hidden" name="' . $instancename . '" value="' . $mod->get_icon_url() . '" />';
                        echo '<a ' . $linkcss . $mod->extra . $onclick .
                        ' href="' . $url . '"><img src="' . $mod->get_icon_url() .
                        '" class="activityicon" style="display:none;"  alt="' . $modulename . '" /> ' .
                        $accesstext . '<span class="instancename" style="display:none;">' .
                        $instancename . $altname . '</span></a>';
            ...
            if ($completion != COMPLETION_TRACKING_NONE && isloggedin() &&
                ...
                if ($completionicon) {
                    ...
                    if ($completion == COMPLETION_TRACKING_MANUAL && !$isediting) {
                        ...
                        if ($imgsrc == $OUTPUT->pix_url('i/completion-auto-y')) {
                            echo "Done (100%)&nbsp;";
                        } else {
                            echo "Done (100%)&nbsp;";
                        } //add arizan
                ...
                if ($url = $mod->get_url()) {
                    ...
                    if ($modulename == 'SCORM package') {
                        ...
                        if ($USER->id === '2') {
                            ...
                            echo '<a target="_blank"' . $linkcss . $mod->extra . $onclick .
                            ' href="' . $url3 . '"><img src="' . $mod->get_icon_url() .
                            '" class="activityicon" alt="' .
                            $modulename . '" /> ' .
                            $accesstext . '<span class="instancename">' .
...
function create_course($data, $editoroptions = NULL) {
    ...
    if ($editoroptions) {
        ...
        // course details // add by arizan
        $data->summarydetails = '';
        $data->summarydetails_format = FORMAT_HTML;
    }
    ...
    if ($editoroptions) {
        ...
        // course details // add by arizan
        $data = file_postupdate_standard_editor($data, 'summarydetails', $editoroptions, $context, 'course', 'summarydetails', 0);
        $DB->set_field('course', 'summarydetails', $data->summarydetails, array('id' => $newcourseid));
        $DB->set_field('course', 'summarydetailsformat', $data->summarydetails_format, array('id' => $newcourseid));
    }
...
function update_course($data, $editoroptions = NULL) {
    ...
    if ($editoroptions) {
        $data = file_postupdate_standard_editor($data, 'summary', $editoroptions, $context, 'course', 'summary', 0);
        $data = file_postupdate_standard_editor($data, 'summarydetails', $editoroptions, $context, 'course', 'summarydeails', 0); // Course details// add by aarizan
    }
