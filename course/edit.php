...
if($categoryid=='3'){ 
	require_once('edit_form_exammodule.php');
}else if($categoryid=='9'){
	require_once('edit_form_moduletest.php');		
}else{	require_once('edit_form.php');	}
...
if (!empty($course)) {
    ...
    $course = file_prepare_standard_editor($course, 'summarydetails', $editoroptions, $coursecontext, 'course', 'summarydetails', 0); // course details // add by arizan

} else {
    $course = file_prepare_standard_editor($course, 'summary', $editoroptions, null, 'course', 'summary', null);
    $course = file_prepare_standard_editor($course, 'summarydetails', $editoroptions, null, 'course', 'summarydetails', null);  // course details // add by arizan
}

// first create the form
//echo $categoryid;
if($categoryid=='3'){ 
	$editform = new course_edit_form_exammodule(NULL, array('course'=>$course, 'category'=>$category, 'editoroptions'=>$editoroptions, 'returnto'=>$returnto));
}
else if($categoryid=='9'){ 
	$editform = new course_edit_form_moduletest(NULL, array('course'=>$course, 'category'=>$category, 'editoroptions'=>$editoroptions, 'returnto'=>$returnto));
}else{
...
if (!empty($course->id)) {
    ...
    //$PAGE->navbar->add($strcategories, new moodle_url('/course/category.php?id=1&categoryedit=on&sesskey=x3OeAgIuFp'));
