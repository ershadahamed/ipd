...
//--------------------------------------------------------------------------------
		...
		$mform->addElement('text','idnumber', get_string('programcodename'),'maxlength="100"  size="10"');
		//$mform->addHelpButton('idnumber', 'idnumbercourse');
		$mform->addRule('idnumber', get_string('programcodename'), 'required', null, 'client'); //added by arizan
        ...
        $level['2'] = 'Level 2';
        $level['3'] = 'Level 3';			
		$mform->addElement('hidden', 'course_level', get_string('course_level'), $level);
		...
		if(($category->id) == '3'){
			//$mform->addElement('select','examcentrecode', get_string('examcentrecode'), $examcentrecode);
		...
		// course details
        $mform->addElement('editor','summarydetails_editor', get_string('coursesummary').'&nbsp; details', null, $editoroptions);
        $mform->addHelpButton('summarydetails_editor', 'coursesummary');
        $mform->setType('summarydetails_editor', PARAM_RAW);

        if (!empty($course->id) and !has_capability('moodle/course:changesummary', $coursecontext)) {
            $mform->hardFreeze('summarydetails_editor');
        }		
        ...
        $choices['11'] = '11 month';
        $choices['12'] = '12 month';			
		$mform->addElement('select', 'duration', get_string('duration'), $choices);
        $mform->addHelpButton('duration', 'duration');
		$mform->setDefault('duration', 3);
...
//--------------------------------------------------------------------------------
        ...
        if ($roles = get_all_roles()) {
            ...
            foreach ($roles as $role) {
                $mform->addElement('hidden', 'role_'.$role->id, get_string('yourwordforx', '', $role->name));
