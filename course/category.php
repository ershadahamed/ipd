<?php
    ...
    if ($editingon && can_edit_in_category()) {
		...
		//modified by arizanabdullah
       // admin_externalpage_setup('coursemgmt_category', '', $urlparams, $CFG->wwwroot . '/course/category.php');
...
/// Print the category selector
	...
	echo '<b>'.$strcategories.': '.$category->name.'</b>';
    echo '</div>';
...
/// Print current category description
    ...
    if ($editingon && has_capability('moodle/category:manage', $context)) {
        ...
        echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string('manageallcurriculum'), 'get'); 
		echo $OUTPUT->single_button(new moodle_url('editcategory.php', $options), get_string('addnewcurriculum'), 'get');  
...
/// Print out all the courses
    ...
    if (!$courses) {
        ...
        $strcourses = get_string('listof').' '.$category->name;
        ...
        foreach ($courses as $acourse) {
            ...
            if ($editingon) {
                ...
                if (has_capability('moodle/course:update', $coursecontext)) {
					//add by arizanabdullah on 03012012
					if(($category->id) == '3'){
                    echo $OUTPUT->action_icon(new moodle_url('/course/edit_exam.php',
                            array('id' => $acourse->id, 'category' => $id, 'returnto' => 'category')),
                            new pix_icon('t/edit', $strsettings));
					}
					
					elseif(($category->id) == '2'){
                    echo $OUTPUT->action_icon(new moodle_url('/course/edit_moduletest.php',
                            array('id' => $acourse->id, 'category' => $id, 'returnto' => 'category')),
                            new pix_icon('t/edit', $strsettings));
					}
        ...
        if ($abletomovecourses) {
			...
			//add by arizan abdullah 10/1/2012
			$movetocategories[$category->id] = get_string('moveselectedcoursesto');
...
/*if (has_capability('moodle/category:manage', $systemcontext)) {
    ...
    /// Print button to create a new course
		...
		//add by arizanabdullah 04012012
		if($category->id =='2'){
			echo $OUTPUT->single_button(new moodle_url('edit_moduletest.php', $options), get_string('addnewcourse').' '.strtolower($category->name), 'get');
		}
		elseif($category->id =='3'){
			echo $OUTPUT->single_button(new moodle_url('edit_exam.php', $options), get_string('addnewcourse').' '.strtolower($category->name), 'get');
		}
		elseif($category->id =='1'){
			echo $OUTPUT->single_button(new moodle_url('edit.php', $options), get_string('addnewcourse').' '.strtolower($category->name), 'get');
		}
		else{
			echo $OUTPUT->single_button(new moodle_url('edit.php', $options), get_string('addnewcourse').' '.strtolower($category->name), 'get');
		}	
