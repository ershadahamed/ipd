...
class course_edit_form_moduletest extends moodleform {
    protected $course;
    protected $context;
...
//--------------------------------------------------------------------------------
        ...
        $mform->addElement('text','fullname', get_string('fullnamecourse2'),'maxlength="254" size="50"');
        ...
        $mform->addElement('text', 'shortname', get_string('shortnamecourse2'), 'maxlength="100" size="20"');
		...
		//Course Type
        $type = array();
        $type['fullpackage'] = 'Full package';
        $type['coursecontent'] = 'Course Content';
        $type['moduletest'] = 'Module Test';			
		$mform->addElement('hidden', 'course_type', get_string('course_type'), $type);
		$mform->addHelpButton('course_type', 'course_type');
		$mform->setDefault('course_type', 'fullpackage'); /*selected value for default*/
        ...
        $level['2'] = 'Level 2';
        $level['3'] = 'Level 3';			
		$mform->addElement('hidden', 'course_level', get_string('course_level2'), $level);
		$mform->addHelpButton('course_level', 'course_level');
		$mform->setDefault('course_level', '1'); /*selected value for default*/	

		//arizan 08/12/11//*****************add this fill for exam form only
		/*if(($category->id) == '3'){
		include('../manualdbconfig.php');
		$examcentrelist=array();
		$examcentrelist[0]=' ----- Exam Centre Code ----- ';
		$sqlList=mysql_query("Select centre_name, centre_code From {$CFG->prefix}_exam");
		while($result=mysql_fetch_array($sqlList)){
			$code=$result['centre_code'];
			$codeName=$result['centre_code'].' ('.$result['centre_name'].')';
			$examcentrelist[$code]=$codeName;
		}
			$mform->addElement('select','examcentrecode', get_string('examcentrecode'),$examcentrelist);
			//$mform->addElement('text','examcentrecode', get_string('examcentrecode'),'maxlength="100"  size="20"');
			$mform->addHelpButton('examcentrecode', 'examcentrecode');
			$mform->setType('examcentrecode', PARAM_MULTILANG);
		}*/	
		//*********************************************************//
        ...
        $mform->addElement('text','idnumber', get_string('idnumbercourse2'),'maxlength="100"  size="20" value="CIFA'.$autoid.'"');
        ...
        $mform->addElement('editor','summary_editor', get_string('coursesummary2'), null, $editoroptions);
        ...
        //$mform->addElement('date_selector', 'startdate', get_string('startdate2'));
		$mform->addElement('hidden', 'startdate', get_string('startdate2'));
        ...
        $choices['11'] = '11 month';
        $choices['12'] = '12 month';			
		$mform->addElement('hidden', 'duration', get_string('duration2'), $choices);
        $mform->addHelpButton('duration', 'duration');
...
//--------------------------------------------------------------------------------
        $mform->addElement('header','', get_string('groups', 'group'));//header to hidden

        $choices = array();
        $choices[NOGROUPS] = get_string('groupsnone', 'group');
        $choices[SEPARATEGROUPS] = get_string('groupsseparate', 'group');
        $choices[VISIBLEGROUPS] = get_string('groupsvisible', 'group');
        $mform->addElement('select', 'groupmode', get_string('groupmode', 'group'), $choices); //select to hidden
        ...
        $mform->addElement('select', 'groupmodeforce', get_string('groupmodeforce', 'group'), $choices); //select to hidden
        ...
        $mform->addElement('select', 'defaultgroupingid', get_string('defaultgrouping', 'group'), $options); //select to hidden
