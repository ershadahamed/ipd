<?php
    ...
    if ($editingon && can_edit_in_category()) {
		...
		//admin_externalpage_setup('coursemgmt_category', '', $urlparams, $CFG->wwwroot . '/course/category_3.php');
...
/// Print the category selector
	...
	echo '<br/>';
    /*$select = new single_select(new moodle_url('category_2.php'), 'id', $displaylist, $category->id, null, 'switchcategory');
	$select->set_label($strcategories.':');
    echo $OUTPUT->render($select);*/
	echo '<p><b>'.$strcategories.': '.$category->name.'</b></p>';
    echo '</div>';
...
/// Print current category description
    ...
    if ($editingon && has_capability('moodle/category:manage', $context)) {
		...
		$options = array('parent'=>0);
                echo $OUTPUT->single_button(new moodle_url('index.php', $options), 'Manage All Category', 'get'); 
        ...
        foreach ($courses as $acourse) {
            ...
            if ($editingon) {
                ...
                if (has_capability('moodle/course:update', $coursecontext)) {
					...
					//add by arizan abdullah 04/01/2012
