						...
						if($_POST['sahbeli']){						
							...
							while($sgetuser=mysql_fetch_array($selectgetuser)){
								...
								if($ucount=='0'){
									...
									$selectusertoken=mysql_query("SELECT * FROM {$CFG->prefix}user_accesstoken WHERE userid='".$ucadidateid."' AND courseid='".$eexam['courseid']."'");
									$cusertoken=mysql_num_rows($selectusertoken);
									if($cusertoken=='0'){
										$access_token=uniqid(rand());
										$tokencreated=strtotime('now'); //token start
										$tokenexpiry=strtotime(date('d-m-Y H:i:s',$tokencreated) . " + 1 year"); //token expiry
									...
									if(($sqlrow['traineeid']==$yes) && ($sqlrow['productid']==$getcourseid)){
										...
										$expired2=strtotime($signupdate . " + 3 month"); //subscribe end
										...
										$u_timeend=strtotime(date('M d, Y',$timeendcourse) . " + 3 month");
