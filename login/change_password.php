...
if ($mform->is_cancelled()) {
	...
	$sqlupdateusr=mysql_query("
		...
		SET 
			...
			countryemploy='".$_POST['countryemploy']."'
		WHERE 
			id='".$USER->id."'
		");
	//echo arizan disember 2012	
    ...
    //$PAGE->navbar->add($fullname, new moodle_url('/user/view.php', array('id'=>$USER->id, 'course'=>$course->id))); /* by arizan */
    //$PAGE->navbar->add($strpasswordchanged);
    $PAGE->set_title($strpasswordchanged);
    $PAGE->set_heading($COURSE->fullname);
	$PAGE->set_pagelayout('change_password'); //add by arizan
    echo $OUTPUT->header();
