    ...
    public function render_end_bits(mod_quiz_renderer $output) {
		...
		*/
		/*return html_writer::start_tag('form',
                ...
                'id' => 'responseform')) .
				html_writer::empty_tag('input', array('type' => 'submit', 'name' => get_string('endtest', 'quiz'),
                'value' => get_string('endtest', 'quiz'), 'onclick'=>'this.form.action="'.$this->attemptobj->summary_url().'"')) .
                $output->countdown_timer() .
                $this->render_restart_preview_link($output);*/
				
		$a=html_writer::start_tag('form',
                array('method' => 'post',
                'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8',
                'id' => 'responseform'));
		$a .= html_writer::start_tag('div', array('style'=>'display:inline-block; margin-right:0.5em;'));
		$a.=html_writer::empty_tag('input', array('type' => 'submit', 'name' => get_string('endtest', 'quiz'),
		'value' => get_string('endtest', 'quiz'), 'onclick'=>'this.form.action="'.$this->attemptobj->summary_url().'"'));
		$a .= html_writer::end_tag('div');
		
		$a .= html_writer::start_tag('div', array('style'=>'display:inline-block;'));
		$a.=html_writer::empty_tag('input', array('type' => 'submit', 'name' => get_string('finish', 'quiz'),
		'value' => get_string('finish', 'quiz'), 'onclick'=>'this.form.action="'.$this->attemptobj->summary_url().'"'));	
		$a .= html_writer::end_tag('div');
		$a.=$output->countdown_timer();
		$a.=$this->render_restart_preview_link($output);
