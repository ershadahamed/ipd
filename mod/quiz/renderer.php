...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function finish_review_link($url) {
        if ($this->page->pagelayout == 'popup') {
            ...
            $this->page->requires->js_init_call('M.mod_quiz.secure_window.init_close_button', array($url), quiz_get_js_module());
    ...
    public function countdown_timer() {
        return html_writer::tag('div', get_string('timeleft', 'quiz') . '<br/> ' .
                        html_writer::tag('span', '', array('id' => 'quiz-time-left')), array('id' => 'quiz-timer'));
    }
    ...
    public function navigation_panel(quiz_nav_panel_base $panel) {
        ...
        $output = '';
        $output .= $this->countdown_timer(); //added by arizan
        $output .= html_writer::tag('div', 'Question numbers', array('id' => 'user-picture', 'class' => 'clearfix'));
    ...
    protected function render_quiz_nav_question_button(quiz_nav_question_button $button) {
        ...
        return html_writer::link($button->url, html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        html_writer::tag('span', '', array('class' => 'trafficlight')) .
                        get_string($qnostring, 'quiz', $a), array('class' => implode(' ', $classes), 'id' => $button->id,
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        foreach ($slots as $slot) {
            $output .= $attemptobj->render_question($slot, false, $attemptobj->attempt_url($slot, $page));
        }
        ...
        $currentpage = $page - 1;
        $output .= html_writer::start_tag('div', array('class' => 'submitbtns'));
        if ($page >= '1') { //added by arizan 01012013
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'Previous', 'class' => 'id_defaultbutton',
                        'value' => 'Previous', 'onclick' => 'this.form.action="attempt.php?attempt=' . $attemptobj->get_attemptid() . '&page=' . $currentpage . '"'));
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                    'value' => get_string('next')));
        $output .= html_writer::end_tag('div');
    ...
    public function summary_page($attemptobj, $displayoptions) {
        $h2_open = '<h2 style="font-weight:400;text-transform:none; text-align:center;">';
        $h2_close = '</h2>';
        $output = '';
        $output .= $this->header();
        if ($attemptobj->get_quiz_name() === 'CIFATM Foundation Mock Exam') {
            $getquizname = get_string('quizname_set', 'quiz'); //edited by arizan
            $output .= $h2_open . 'CIFA<font size="1px"><sup>TM</sup></font> Foundation Mock Exam Script Summary' . $h2_close;
        } else {
            /* $output .= $this->heading(format_string($attemptobj->get_quiz_name()));
              $output .= $this->heading(get_string('summaryofattempt', 'quiz'), 3); */

            //$output .= $h2_open.format_string($attemptobj->get_quiz_name()).$h2_close;
            $output .= $h2_open . get_string('summaryofattempt', 'quiz') . $h2_close;
        }
    ...
    public function view_page($course, $quiz, $cm, $context, $infomessages, $viewobj, $buttontext, $preventmessages) {
        ...
        $output .= $this->view_attempt_button($course, $quiz, $cm, $context, $viewobj, $buttontext, $preventmessages);
        return $output;
    }
     ...
     * @param array $messages Array containing any messages
     */
    /* ORI public function view_information($course, $quiz, $cm, $context, $messages) {
      global $CFG;
      $output = '';
    ...
    public function view_information($course, $quiz, $cm, $context, $messages) {
        ...
        $CIFATM = 'CIFA<sup><font size="1px">TM</font></sup>';
        $h2_open = '<h2 style="font-weight:400;text-transform:none; text-align:center;">';
        $h2_close = '</h2>';
        $output = '';
        // Print quiz name and description//When you are ready, click on the "Proceed" button below.
        // $output .= $this->heading('::Instruction to Candidates::'); //change by arizan 1/1/2013
        $output .= $h2_open . '::Instruction to Candidates::' . $h2_close; //change by arizan 1/1/2013

        if ($course->category == '1') {
            // $output .= $this->heading(format_string($quiz->name));
            if (($quiz->name) == 'CIFATM Foundation Mock Exam') {
                $mockname = 'CIFA<sup><font size="1px">TM</font></sup> Foundation Mock Exam';
                $output .= $h2_open . $mockname . $h2_close;
            } else {
                $output .= $h2_open . format_string($quiz->name) . $h2_close;
            }
        } else {
            //$output .= $this->heading(format_string($quiz->name));
            $output .= $h2_open . format_string($quiz->name) . $h2_close;
	    ...
	    //if(strpos($USER->email,'adib')!== false){ 

//mmn off 26/09/2016 if($USER->orgtype=='7' || $USER->orgtype=='8'|| $USER->orgtype=='9' || $USER->orgtype=='10'|| $USER->orgtype=='11' || $USER->orgtype=='12'){
if($USER->orgtype=='7' || $USER->orgtype=='8'|| $USER->orgtype=='9' || $USER->orgtype=='1'|| $USER->orgtype=='3'){

            $output .= '<div style="text-align:center;"><p>Pass mark: 75%</p></div>';
			}else{
			$output .= '<div style="text-align:center;"><p>Pass mark: 60%</p></div>';
			}
			//mmn end
            }
        }
        ...
        if (trim(strip_tags($quiz->intro))) {
            ...
            /* if($course->id == '12'){
              $output .= $this->box(get_string('examinstructions', 'quiz'), 'generalbox', 'intro');
              } */
            if ($course->category == '3') {
//mmn off 26/09/2016 if($USER->orgtype=='7' || $USER->orgtype=='8'|| $USER->orgtype=='9' || $USER->orgtype=='10'|| $USER->orgtype=='11' || $USER->orgtype=='12'){
if($USER->orgtype=='7' || $USER->orgtype=='8'|| $USER->orgtype=='9' || $USER->orgtype=='1'|| $USER->orgtype=='3'){
                $output .= $this->box(get_string('testinstructionsadib', 'quiz'), 'generalbox', 'intro');
}else{
$output .= $this->box(get_string('testinstructions', 'quiz'), 'generalbox', 'intro');
}
            } else if ($course->category == '9') {
                $a = format_string($quiz->name);
                $output .= $this->box(get_string('mockexaminstructions', 'quiz', $a), 'generalbox', 'intro');
            } else if ($course->category == '1') {
                $a = format_string($quiz->name);
                $output .= $this->box(get_string('mockexaminstructions', 'quiz', $a), 'generalbox', 'intro');
            } else {
                if($USER->orgtype=='7' || $USER->orgtype=='8'|| $USER->orgtype=='9' || $USER->orgtype=='10'|| $USER->orgtype=='11' || $USER->orgtype=='12'){
$output .= $this->box(get_string('testinstructionsadib', 'quiz'), 'generalbox', 'intro');
}else{
$output .= $this->box(get_string('testinstructions', 'quiz'), 'generalbox', 'intro');
}
        ...
        // $output .= $this->box($this->access_messages($messages), 'quizinfo'); //disable by arizan
        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/quiz:viewreports', $context)) {
        ...
        foreach ($viewobj->attempts as $attempt) {
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attempt->preview && $viewobj->numattempts > 1 && !is_null($viewobj->mygrade) && $attemptgrade == $viewobj->mygrade && $quiz->grademethod == QUIZ_GRADEHIGHEST) {
    ...
    public function view_result_info($quiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->feedbackcolumn) {
            ...
            $resultinfo .= html_writer::tag('p', quiz_feedback_for_grade($viewobj->mygrade, $quiz, $context), array('class' => 'quizgradefeedback')) . "\n";
        }
