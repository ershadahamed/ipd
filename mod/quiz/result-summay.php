...
?>
	...
	<?php 
		...
		//mmn add to get passgrade
				$qryge = "SELECT a.orgtype FROM mdl_cifauser a, mdl_cifaorganization_type b
					 where a.id='".$USER->id."' AND a.orgtype=b.id AND b.name LIKE '%ADIB%'";
				$sqlge = mysql_query($qryge);
				$rsge = mysql_fetch_array($sqlge);
				$rowge = mysql_num_rows($sqlge);
				
				$qryed = "SELECT * FROM mdl_cifaquiz_grades WHERE quiz='26' AND userid='".$USER->id."' AND timemodified>'1455494400'";
				...
				$usergrade = round($q1final_result);
		//mmn end get passgrade
		if($today == $beforeexpiry){
		if($_GET['courseid']=='64'){ if($usergrade>=$passgrade){
		$linkcontact=$CFG->wwwroot. '/contactus/upload_index.php';
		// email to commenter
			$to = $USER->email;
			$subject = "Announcement: CANDIDATE ID - Get Your SHAPE IPD Certificate";
			$message = "
				<html>
				<head>
