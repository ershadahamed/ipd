...
class quiz_access_manager {
    ...
    public function make_review_link($attempt, $canpreview, $reviewoptions) {
        ...
        if ($this->securewindow_required($canpreview)) {
            ...
            /*mmn off JUNE 2012return '<a href="' . $this->_quizobj->review_url($attempt->id) . '" title="' .
                    get_string('reviewthisattempt', 'quiz') . '">' . $linktext . '</a>';*/
			return '<a href="' . $CFG->wwwroot.'/mod/quiz/review2.php?id='.$_GET['id'] . '" title="' .
                    get_string('reviewthisattempt', 'quiz') . '">' . $linktext . '</a>'; //mmn add to redirect to new result page
        }
    }
