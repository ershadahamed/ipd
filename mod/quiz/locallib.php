...
function quiz_question_edit_button($cmid, $question, $returnurl, $contentaftericon = '') {
    global $CFG, $OUTPUT;
	global $USER, $DB; //add by arizan 20121030
    ...
    if ($action) {
		...
		$queryrole  = $DB->get_records('role_assignments',array('userid'=>$USER->id)); // add by arizan
		foreach($queryrole as $qrole){ }  
		
		if(($USER->id =='2') || ($qrole->roleid =='3') || ($qrole->roleid =='4')){ //edited by arizan 20121030		
...
function quiz_question_preview_button($quiz, $question, $label = false) {
	...
	if(($USER->id =='2') || ($qrole->roleid =='3') || ($qrole->roleid =='4')){ // add by arizan
