...
$PAGE->set_pagelayout('incourse_layer1');

$urlcourse = new moodle_url('/coursesindex.php', array('course'=>$course->id)); //added by arizan
$testlink = new moodle_url('/mod/quiz/view.php', array('id'=>$_GET['id'])); //added by arizan

$PAGE->navbar->add(get_string('mytraining'), $urlcourse)->add(get_string('activetrainings'), $urlcourse)->add(format_string($quiz->name), $urlcourse);
if(($COURSE->category)=='9'){ // mock test
	$PAGE->navbar->add(get_string('mocktest'), $testlink);  // arizan abdullah 
}else{
	$PAGE->navbar->add(get_string('finaltest'), $testlink); // final test
}
...
if ($quiz->attempts != 1) {
   // $infomessages[] = get_string('gradingmethod', 'quiz',
            //quiz_get_grading_option_name($quiz->grademethod)); //remove by arizanabdullah 11jan2013
}
