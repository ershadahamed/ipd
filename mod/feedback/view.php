...
$PAGE->navbar->add(get_string('mytraining'))->add(get_string('activetrainings'))->add($evaluation);//add by arizan dec 2013
echo $OUTPUT->header();
...
// $previewlnk = '<a href="'.$CFG->wwwroot.'/mod/feedback/print.php?id='.$id.'">'.$previewimg.'</a>'; // arizan

echo $OUTPUT->heading(format_text($feedback->name.' '.$previewlnk)). '<br/>';
...
//echo format_module_intro('feedback', $feedback, $cm->id); // by arizan
echo $OUTPUT->box_end();
...
if($feedback_complete_cap) {
    ...
    if(($feedback->timeopen > $checktime) OR ($feedback->timeclose < $checktime AND $feedback->timeclose > 0)) {
			...
			echo $OUTPUT->continue_button($CFG->wwwroot.'/index.php?id='.$_GET['id']);
        echo $OUTPUT->box_end();
        echo $OUTPUT->footer();
        exit;
    }
    ...
    if($feedback_can_submit) {
        ...
        if($courseid) {
            //echo $OUTPUT->continue_button($CFG->wwwroot.'/course/view.php?id='.$course->id); //arizan
			echo $OUTPUT->continue_button($CFG->wwwroot.'/index.php?id='.$_GET['id']);
        }else {
            //echo $OUTPUT->continue_button($CFG->wwwroot.'/course/view.php?id='.$course->id);
			echo $OUTPUT->continue_button($CFG->wwwroot.'/index.php?id='.$_GET['id']);
