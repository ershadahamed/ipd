<?php
    ...
    */
    ...
    if($course->id == SITEID) {
        ...
        // $PAGE->set_pagelayout('incourse'); // arizan
		$PAGE->set_pagelayout('buy_a_cifa');
    }

	//create by arizan abdullah //aa
	$evaluation=get_string('evaluation');
	$mytraining=get_string('mytraining');
	$activetrainings=get_string('activetrainings');
	if($USER->id !='2'){
		$PAGE->navbar->add($mytraining)->add($activetrainings)->add($evaluation);
	}else{
	...
	$PAGE->set_pagelayout('buy_a_cifa'); // add by arizan
    ...
    if(($feedback->timeopen > $checktime) OR ($feedback->timeclose < $checktime AND $feedback->timeclose > 0)) {
        echo $OUTPUT->box_start('generalbox boxaligncenter');
            echo '<h2 style="text-align:center;"><font color="red">'.get_string('feedback_is_not_open', 'feedback').'</font></h2>';
			echo $OUTPUT->home_button($CFG->wwwroot.'/index.php?id='.$course->id);
    ...
    if($feedback_can_submit) {
        ...
        // echo $OUTPUT->heading(format_text($feedback->name)); // remove by arizan
        ...
        if(isset($savereturn) && $savereturn == 'saved') {
            if($feedback->page_after_submit) {
                ...
                echo '<p align="center"><b><font color="green"><em>'.get_string('entries_saved','feedback').'</em></font></b></p>';
            ...
            if($feedback->site_after_submit) {
                echo $OUTPUT->home_button(feedback_encode_target_url($feedback->site_after_submit));
            }else {
                if($courseid) {
                    if($courseid == SITEID) {
                        echo $OUTPUT->home_button($CFG->wwwroot);
                    }else {
            ...
            if(is_array($feedbackitems)){
                ...
                echo '<br/><br/><div style="display:inline;">';
				if(isset($startitem)) {
                ...
                if($lastbreakposition >= $maxitemcount) { //last page
					// $go=$CFG->wwwroot. 'mod/feedback/complete.php';
					$go_home=$CFG->wwwroot .'/index.php';
					// echo '<button onClick="this.form.action='.$go_home.'" type="submit" style="margin-right:0.5em;">'.get_string('cancel').'</button>';
        ...
        echo $OUTPUT->box_start('generalbox boxaligncenter');
            echo '<h2 style="text-align:center;"><font color="red">'.get_string('this_feedback_is_already_submitted', 'feedback').'</font></h2>';
            //echo $OUTPUT->continue_button($CFG->wwwroot.'/course/view.php?id='.$course->id);
			echo $OUTPUT->home_button($CFG->wwwroot.'/index.php?id='.$course->id);
        echo $OUTPUT->box_end();
    }
